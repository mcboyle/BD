<!-- verified-against: v3.66.159. RETIRED ENVIRONMENT: this file documents the pre-git Claude.ai kit sandbox and is a historical record, not current setup. SANDBOX_OPS_NOTES.md and STATE.json, referenced by an earlier version of this comment, no longer exist. For current behaviour see CLAUDE.md section 5, docs/repo/ENVIRONMENT_PROVISIONING.md and project-knowledge/KNOWN_FLAKES.md. Standing footgun that survives the change: a stale frontend/src or a missing frontend/dist bundle gives a silent 503 on the SPA -- rebuild with `cd frontend && npm ci && npm run build` before trusting the UI. -->
