<!-- verified-against: v3.66.159; SUPERSEDED IN PART by SANDBOX_OPS_NOTES.md (176-verified), KNOWN_FLAKES.md, and the STATE.json/bd-preflight flow. Where they differ, the newer doc wins. NEW footgun: bd-install/image can present a STALE frontend/src — run bd-preflight before any SPA build. -->
...
1. **`/mnt/transcripts/journal.txt`** — *(LEGACY — usually empty now; current continuity is the per-session `version.zip` + `STATE.json`. Kept only as a fallback.)* catalog. Read the tail; each entry names a
