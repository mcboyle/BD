...
Where things live and how a capture becomes a download. This is the sole live
human-authored architecture/design map; executable safety facts live in
`../guards.json`, `../FOOTGUNS.json`, and `../INVARIANTS.json` and
current topology is generated by the route, dependency, function, import, and
GUI-parity artifacts. Never copy their volatile counts here. The completed-program import graph is archived at
...
the current tree before making architecture decisions.
...
## runner.py -- kernel and behavior mixins

`runner.py` is the kernel, with `runner_util.py` helpers and behavior mixins:
...
`resume_site_keepers` must NEVER exist. Machine-enforced invariants live in root
`INVARIANTS.json`.

## app.py and blueprints, by functional domain

Routes are predominantly grouped in `app_<group>.py`; the exact current
population belongs to generated route/GUI artifacts. Blueprint registration is
fail-closed when a required module cannot load. A leaf blueprint imports flask/stdlib, never `app`;
...
The `deep_detect/` package is the deep page/player analysis layer feeding recognition.

## Safety and generated trust surfaces

- `guards.json` is the only roster and digest authority for the seven
  byte-identical release guards; `bd-guardcheck` is the only checker CI and
  provisioning invoke.
- `FOOTGUNS.json` is the operational footgun authority loaded by
  `bd-footguns` and packaged once beside the standalone tool.
- `INVARIANTS.json` is the concrete guarded-invariant authority. Every row
  names a current guard test; missing or unguarded rows fail its checker.
- `DEFECT_PATTERN_CATALOG.md` documents detector classes generated from the
  executable detector metadata; it is not a second runtime registry.
- `bd-regen-order` owns the generation order for GUI parity, route index,
  endpoint catalog, dependency graph, function index, pins, and static KB.
  SPA wiring must remain a literal `/api/...` string so parity analysis can
  observe it.
- Routine deployment conditions and rollback live in
  `../docs/repo/FRESH_HOST_BRINGUP.md`; a Git update alone neither clears
  bytecode, regenerates ignored artifacts, builds `frontend/dist`, nor restarts
  the service.
