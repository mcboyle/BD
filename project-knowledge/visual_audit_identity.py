...
def load_validated_manifest(
    ...
    # ORDER IS LOAD-BEARING. Row 308 validates the RELEASE IDENTITY; row 309
    # validates the expected VIEW POPULATION. Identity must run first: 309's
    # contract rejects any incomplete manifest, so running it first would make
    # every identity test fail with 'missing 100 expected rows' before the
    # version was ever compared -- the population error would mask the identity
    # error it is supposed to sit beside, not in front of.
    version = validate_manifest_release(rows, repo_root)
    from capture_manifest_contract import validate_manifest
    return validate_manifest(rows), version
