...
- **env-var-opt-in-trips-the-env-tranche-gate** *(fresh @674, ×2 on-stash)* — a backend-only
  ...
  *open* env_var → `test_v3_66_319::test_no_open_env_vars_remain` fails and `open_runtime_tunable`
  ...
  `import_graph_gate --update` ONCE per train, in the train (and on every restack), never in the worker cut that adds the edge -- the worker declares it (H102, H116); and *removing* an import stales DEPENDENCY_GRAPH too
...
- **the import gate tracks function-local imports too** *(@582).* `dependency_graph.py::_internal_imports`
  ...
  already exists*; a genuinely NEW module→module dependency is flagged and must be DECLARED by the worker
  cut; the train re-freezes ONCE (`import_graph_gate --update` + `dependency_graph` regen; H102, H116).
