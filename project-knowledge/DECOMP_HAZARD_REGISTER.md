...
|---|---|---|---|---|---|
| **H-01** | Historical overlay stale shadow (retired) | Pre-git deployment used `unzip -o` for a `.py`→package split | The obsolete path could survive an overlay and shadow the package. The sanctioned git-reset deploy removes tracked deletions, so this is historical evidence rather than a current mitigation. | Historical | Current deployments use `scripts/deploy.sh`; do not restore overlay-specific shims or removal steps. |
...
| **H-05** | Manual-bump PIN drift | hand-editing `bulk_downloader/__init__.py:26` without regen | `test_pin_index_in_sync` + `kb_golden_questions(what-pins)` + `version_pins_match_live` fail | Sandbox-INVISIBLE | Every version bump runs `python tools/build_pin_index.py` / `bd_regen` |
...
| **H-14** | Lazy-accessor state back-edge boundary | Current re-derivation finds 145 `_app_*` accessors across 75 `app_*.py` files | Blueprints deliberately resolve shared application state through a lazy accessor, retaining a hub-and-spoke back-edge. Current import-graph and focused behavior gates own cycle safety; this is accepted architecture, not an unnumbered decomposition task. | Architectural | Preserve the current gates. Any future state-hoisting proposal requires a new atomic backlog row with an exact accessor denominator and RED behavior proof. |
