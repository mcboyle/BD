...
treats as job identity) and, when supplied, a query-insensitive identity of
the media URL whose bytes may occupy the path, so:

  * a DIFFERENT job can never be handed a staging path that is already claimed;
  * the SAME job -- an interrupted download restarted later -- resumes its own
    ``.part`` only for the same media resource; rotating CDN signatures match,
    while a different host or path refuses before a Range request.
...
def _read_resource(owner_path: Path, record) -> str | None:
    ...
    older writer established that these were this job's bytes, and refusing
    every interrupted download during deployment would discard valid resumes.
    """
