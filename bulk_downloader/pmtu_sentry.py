...
def compute_tcp_mss(
    ...
    return max(0, mtu - ip_hdr - tcp_hdr)
...
def clamp_syn_mss(
    ...
    """
    if advertised_mss < 0:
        raise ValueError(f"advertised_mss must be non-negative, got {advertised_mss}")
    max_allowed = compute_tcp_mss(path_mtu, ip_version=ip_version)
    return min(advertised_mss, max_allowed)


def wireguard_interface_mtu(endpoint_mtu: int, configured_mtu: int | None = None) -> int:
    """Largest WireGuard interface MTU whose encapsulated packets fit the endpoint path MTU.

    A configured MTU already below that ceiling is kept; one above it is clamped down.
    Raises ValueError when the path cannot carry a minimum-size IPv4 packet after overhead.
    """
    path_mtu = int(endpoint_mtu) - TUNNEL_OVERHEADS[TunnelType.WIREGUARD]
    if path_mtu < MIN_IPV4_MTU:
        raise ValueError(
            f"endpoint_mtu {endpoint_mtu} leaves {path_mtu} bytes after WireGuard overhead "
            f"(minimum {MIN_IPV4_MTU})"
        )
    if configured_mtu:
        return min(int(configured_mtu), path_mtu)
    return path_mtu
...
class PMTUSentry:
    ...
    def register_route(
        ...
        effective_mtu = interface_mtu - overhead
    ...
    def record_probe_result(
        ...
        if received_ack:
            ...
            # A loss at or below an ACKed size is ordinary loss (RFC 4821): only the
            # repeated-loss threshold below reacts. A loss above it narrows the search.
            if route.highest_confirmed_mtu < packet_size <= route.effective_mtu:
                if route.highest_confirmed_mtu:
