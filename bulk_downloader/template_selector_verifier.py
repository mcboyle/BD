...
def _serve_offline_document(page: Any, html: str, timeout_ms: int) -> None:
    ...
    # This blocks WebSockets and any protocol the normal request router does
    # not understand. The sole document response above is supplied in-process.
...
def _measure(page: Any, rows: list[dict[str, Any]], field: str) -> None:
    for row in rows:
        ...
        try:
            ...
            # A selector accepted by the compiler but rejected by the browser
            # is still malformed, not a zero-count observation.
...
def _maybe_open_download(page: Any, rows: list[dict[str, Any]], timeout_ms: int) -> dict[str, Any]:
    interaction = {"clicked": False, "click_count": 0, "selector": "", "error": ""}
    row_controls = [row for row in rows if row["role"] == "row" and row["status"] != "MALFORMED"]
    if not row_controls or any((row.get("initial_count") or 0) > 0 for row in row_controls):
        return interaction
    ...
    try:
        ...
        if combined is not None:
            try:
                ...
                # A completed click followed by zero rows is a measured MISS.
                pass
    except Exception as exc:
        interaction["error"] = f"trigger interaction failed: {type(exc).__name__}: {exc}"[:700]
    return interaction
...
def verify_template_source(
    ...
    *,
    ...
    ``template`` may be a committed template id or an explicit synthetic dict
    (the latter is useful for tests and one-off candidate checks).  A URL is
    guarded against private/reserved hosts before launch and on every request.
    """
    if isinstance(template, str):
        template_id = template
        template_data = _committed_template(template)
        if template_data is None:
            return _unknown_report(template_id, [], subject, f"unknown committed template id: {template_id}")
    elif isinstance(template, dict):
        template_data = template
        template_id = str(template.get("id", "<missing-id>"))
    else:
        return _unknown_report("<invalid-template>", [], subject, "template must be an id or mapping")

    entries = enumerate_template_selectors(template_data)
    if not entries:
        return _unknown_report(template_id, entries, subject, "template has no selector denominator")
    try:
        kind, source, html = _read_subject(subject)
    except Exception as exc:
        return _unknown_report(template_id, entries, subject, f"{type(exc).__name__}: {exc}")

    if timeout <= 0:
        return _unknown_report(template_id, entries, subject, "timeout must be greater than zero")
    ...
    if kind == "url":
        ...
        if not allowed:
            return _unknown_report(template_id, entries, subject, f"blocked URL subject: {reason}")
    ...
    try:
        ...
        with cloaked_page(
            ...
            if kind == "html":
                ...
                response = page.goto(source, wait_until="domcontentloaded", timeout=timeout_ms)
                if response is None:
            ...
            if wait_for:
                page.locator(wait_for).first.wait_for(state="attached", timeout=timeout_ms)
