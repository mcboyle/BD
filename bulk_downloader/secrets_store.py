...
def refresh_vault_paths() -> tuple[Path, Path]:
    ...
    if _backend is not None and (secrets != SECRETS_FILE
                                 or meta != SECRETS_META_FILE):
        # A backend is already constructed over the published pair and it
        # reads the module attributes at USE time, so republishing here
        # would silently REDIRECT its next write into a new vault while the
        # old one sits untouched -- this row's own defect through the seam
        # this fix added (measured by the shape lens: set() after a changed
        # BD_INSTALL_DIR succeeded into a new 669-byte file). Refuse the
        # change and keep the pair the backend was built on.
        sys.stderr.write(
            f"  secrets: {_STATE_DIR_ENV} changed while a vault backend is "
            f"active ({SECRETS_FILE} -> {secrets}); the change is IGNORED "
            f"until the service restarts, so the store cannot be redirected "
            f"mid-process.\n")
