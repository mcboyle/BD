...
class DatasetClassificationResult:
    ...
    def is_empty(self) -> bool:
        return self.kind == DatasetKind.EMPTY_COLLECTION
        ...
        *,
        ...
        if isinstance(parsed, list):
            ...
            if item_count == 0:
                signals = [
                    StructuralSignal(
                        signal_type="json_empty_array",
                        score=0.0,
                        description="JSON root array is empty (0 items)",
                    )
                ]
                evidence = ClassificationEvidence(
                    source_type="json",
                ...
                return DatasetClassificationResult(
                    ok=True,
                    kind=DatasetKind.EMPTY_COLLECTION,
                    is_aggregate=False,
                    is_single=False,
                    confidence=0.95,
                    evidence=evidence,
            ...
            is_aggregate = item_count >= threshold or item_count > 1
            kind = DatasetKind.AGGREGATE_DATASET if is_aggregate else DatasetKind.SINGLE_ENTITY
            signals = [
                StructuralSignal(
        ...
        if isinstance(parsed, dict):
            ...
            is_empty_envelope = False
            if found_collection is not None and len(found_collection) == 0:
                is_empty_envelope = True
            elif has_pagination and any(parsed.get(k) == 0 for k in ("total", "total_count", "total_items", "count", "items_count")):
                is_empty_envelope = True

            if is_empty_envelope:
                signals = [
                    StructuralSignal(
                        signal_type="json_empty_collection",
                        score=0.0,
                        description="Empty collection or zero-result envelope detected",
                    )
                ]
                if has_pagination:
                    signals.append(
                        StructuralSignal(
                            ...
                            score=1.0,
                            description="Pagination metadata detected in empty envelope",
                        )
                    )
                evidence = ClassificationEvidence(
                    source_type="json",
            ...
            if found_collection is not None and len(found_collection) > 0:
                ...
                is_aggregate = has_pagination or item_count >= threshold or item_count > 1
                kind = DatasetKind.AGGREGATE_DATASET if is_aggregate else DatasetKind.SINGLE_ENTITY
                signals = [
                    StructuralSignal(
            ...
            if has_pagination:
                signals = [
                    StructuralSignal(
                        signal_type="pagination_metadata",
                        score=1.0,
                        description="Pagination metadata detected without recognized collection list",
                    )
                ]
                evidence = ClassificationEvidence(
                    source_type="json",
