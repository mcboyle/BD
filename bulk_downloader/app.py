...
def _start_session_keepers(site_id=None):
    ...
    def _do_login_for_keeper(site_id, account_idx, cfg):
        ...
        try:
            ...
            # two keepers a two-account site spawns cannot both pass one stale
            # count. Account before contact; a failed attempt spends the site's
...
# Row 1020: the 143 uniform try/import/register_routes blocks that used to run from here down to
# roughly line 7744 are now data in bulk_downloader/route_registry.py. register_all() keeps the
# fail-open behaviour each block had -- one blueprint that cannot import does not take the rest
# down -- and additionally RETURNS which modules registered and which failed, so the set is
# answerable at runtime instead of existing only as source text. The eight non-uniform
# registrations (app_auth+app_oidc, app_tags+app_tool_bridge, app_ytdlp_status+gallery-dl, and
# the five with their own stderr wording) are deliberately left hand-written below.
try:
    from . import route_registry as _route_registry
    ROUTE_REGISTRATION_REPORT = _route_registry.register_all(app)
except Exception as _reg_registry_err:
    import sys as _sys
    _sys.stderr.write(f"[app] route registry not loaded: {_reg_registry_err}\n")
    ROUTE_REGISTRATION_REPORT = {"registered": [], "failed": {}, "counts": {"registered": 0, "failed": 0}}
