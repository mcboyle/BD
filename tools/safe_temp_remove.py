...
def rename_verify_destroy(
    ...
    try:
        ...
        private_path = parent / private
        moved = os.lstat(private_path)
        if (moved.st_dev, moved.st_ino) != expected:
            return False, ("private-name identity mismatch; foreign entry left "
                           f"untouched at {private_path}")
        _destroy_private(private_path)
        if os.path.lexists(private_path):
            return False, f"private entry still exists after removal: {private_path}"
