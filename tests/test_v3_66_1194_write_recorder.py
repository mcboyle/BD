...
def test_selftest_failure_is_a_machine_readable_terminal_verdict(monkeypatch, capsys):
    monkeypatch.setattr(mod, "_split_args", lambda _text: [])
    assert mod.main(["--selftest"]) == 1
    assert capsys.readouterr().out.strip() == "SELFTEST FAIL"
