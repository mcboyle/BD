...
@pytest.mark.parametrize("url,html,reason", (
    ("https://login.example.invalid/login", "<p>Wrong username or password provided</p>", "Rejected login"),
    ("https://login.example.invalid/login", "<p>Wrong <b>username</b> or password provided</p>", "Rejected login"),
    ("https://login.example.invalid/badlogin?ats=x", "<p>Try again</p>", "Rejected login"),
    ("https://elsewhere.example.invalid/members", "<p>Welcome</p>", "cross-origin"),
))
def test_verdict_uses_the_page_after_settling(monkeypatch, tmp_path, url, html, reason):
...
def test_timeout_has_distinct_falsy_outcome(monkeypatch, tmp_path):
    ...
    assert bool(result[0]) is False, result
    assert calls["settle"] == calls["evidence"] == 1, calls
