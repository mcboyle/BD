...
PROXY_SHADOWED: Dict[str, str] = {
    "bulk_downloader/multi_conn.py:186":
        "probe client, proxy=proxy (per-site tunnel). PUBLIC_ONLY policy. The URL is "
        "vetted BEFORE the client by _guard_url(url) on the same executed path, which "
        "returns ssrf_blocked:<reason> and never builds the client.",
    "bulk_downloader/multi_conn.py:518":
        "per-worker chunk client, proxy=proxy (same tunnel as the probe above). "
        "PUBLIC_ONLY policy. Same url, already vetted by _guard_url on the probe path "
        "that scheduled these chunks.",
    "bulk_downloader/runner.py:1181":
        "listing scrape through _cp_proxy, taken only after the VPN tunnel is proved up "
        "(VPNRequiredError fails closed above). PUBLIC_ONLY policy; follow_redirects=False "
        "so the vetted URL is the only URL fetched.",
    "bulk_downloader/runner_extractors.py:629":
        "deep-detect clear-interface probe through _cp_proxy, guarded by the same "
        "VPN-required/tunnel-down refusal. PUBLIC_ONLY policy.",
    "bulk_downloader/runner_manual.py:419":
        "manual download test through _cp_proxy, same VPN-required refusal. PUBLIC_ONLY "
        "policy.",
    "bulk_downloader/runner_transport.py:491":
        "direct_http chunk fetch through proxy_url. PINNED policy -- this site admits "
        "private peers on the base, so the transport was never the thing refusing them.",
    "bulk_downloader/runner_transport.py:1030":
        "probe fetch through proxy_url, VPN-required fails closed above. PINNED policy.",
    "bulk_downloader/session_keeper.py:1216":
        "session check_url through _eff_proxy(...get_socks_url_for_site), VPN-required "
        "fails closed above. PINNED policy.",
    "bulk_downloader/vpn_leak_tests.py:658":
        "the leak probe itself -- its whole purpose is to send a request THROUGH the "
        "tunnel and observe the exit address. PINNED policy.",
}
...
PUBLIC_ONLY_AND_PROXIED: Set[str] = {
    "bulk_downloader/multi_conn.py:186",
    "bulk_downloader/multi_conn.py:518",
    "bulk_downloader/runner.py:1181",
    "bulk_downloader/runner_extractors.py:629",
...
STAR_KWARGS_CONSTRUCTIONS: Set[str] = {
    "bulk_downloader/runner_telemetry.py:300",
    "bulk_downloader/runner_transport.py:2208",
    "bulk_downloader/runner_transport.py:2574",
    "bulk_downloader/runner_transport.py:2804",
}
...
# 3. The registry.
# ---------------------------------------------------------------------------
def test_every_proxied_construction_is_declared_with_a_reason(scan):
    """THE GATE.  A tenth proxied construction is RED until it is declared."""
    ...
    assert not undeclared, (
        ...
        "  " + "\n  ".join(undeclared) + "\n"
        "If that is intended -- the site proxies through a VPN/SOCKS tunnel and "
        "resolving locally would move the lookup off the tunnel -- add it to "
        "PROXY_SHADOWED in this file with the refusal that carries the site instead. "
        "If it is NOT intended, drop the proxy= or route it through the tunnel-aware "
        "path the other nine use.")
    assert not ghosts, (
