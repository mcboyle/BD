...
_EDITABLE_COUNT = 221  # Row 825: +1 kafka_event_streaming_enabled (opt-in LAN Kafka completion events). Row 775: +1 turnstile_one_click (opt-in local Turnstile tick). Row 374: +6 authenticated scene-crawler controls. Row 713: +3 per-site private-host opt-ins (plex_allow_private_host, jellyfin_allow_private_host, stash_allow_private_host). Row 780: +1 listing_url (bare content-URL fallback, restored to CFG_FIELDS). Row 851: +1 use_audio_normalization opt-in.
