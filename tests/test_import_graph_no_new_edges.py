...
def test_the_bd_decomp_function_index_remedy_does_not_violate_the_trio_rule():
    """O592: FG-FUNCTION-INDEX-LINESHIFT told the seat HOLDING THE CUT to run
    tools/build_function_index.py -- but FUNCTION_INDEX.md is a TRIO RULE
    artifact (bd-codex-briefs/COMMON-FLOOR.md), the integrator's on the train,
    never a worker's. Two workers (row770c, row772) hit the contradiction and
    spent a round each; O592 rules the worker RECORDS the drift instead."""
    import importlib.util
    path = _REPO_ROOT / "toolchain" / "bin" / "bd-decomp"
    assert path.exists(), f"{path} is missing"
    spec = importlib.util.spec_from_loader(
        "_o592_bd_decomp",
        importlib.machinery.SourceFileLoader("_o592_bd_decomp", str(path)))
    mod = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = mod
    added = str(path.parent)
    sys.path.insert(0, added)
    ...
    gates = mod.gates_for(_REPO_ROOT, "bulk_downloader/probe.py", False)
    entry = [g for g in gates if g[0] == "FG-FUNCTION-INDEX-LINESHIFT"]
    assert len(entry) == 1, (
        f"precondition: expected exactly one FG-FUNCTION-INDEX-LINESHIFT gate, "
        f"got {[g[0] for g in gates]!r}"
    )
    remedy = entry[0][2]
    assert "OWED TO THE INTEGRATOR" in remedy, (
        f"bd-decomp's FUNCTION_INDEX remedy does not tell the worker to record "
        f"the drift for the integrator: {remedy!r}"
    )
    assert "build_function_index.py` +" not in remedy, (
        f"bd-decomp's FUNCTION_INDEX remedy still hands the worker a bare "
        f"command to regenerate a TRIO RULE artifact itself: {remedy!r}"
    )
    assert "tests/test_function_index_in_sync.py" in remedy, (
        f"bd-decomp's FUNCTION_INDEX remedy dropped the band test: {remedy!r}"
    )


def test_the_bd_decomp_cli_actually_emits_the_function_index_gate():
    """H489 self-mutation floor: gates_for() is also CALLED from report()'s
    plain path, report()'s --json path, and selftest()'s g_plain/g_route --
    a test that only calls gates_for() directly does not prove any of those
    call sites still reach it. Deleting one of those calls must break an
    observable here."""
    bd_decomp = str(_REPO_ROOT / "toolchain" / "bin" / "bd-decomp")
    module = "bulk_downloader/app.py"

    plain = subprocess.run(
        [sys.executable, bd_decomp, "--module", module, "--top", "1"],
        cwd=_REPO_ROOT, capture_output=True, text=True, timeout=60,
    )
    assert plain.returncode == 0, plain.stderr
    assert "FG-FUNCTION-INDEX-LINESHIFT" in plain.stdout, (
        "plain report() no longer emits the FUNCTION_INDEX gate -- its "
        "gates_for() call site was lost"
    )
    assert "OWED TO THE INTEGRATOR" in plain.stdout, (
        "plain report() emits a gate list whose FUNCTION_INDEX remedy is not "
        "the corrected text"
    )

    as_json = subprocess.run(
        [sys.executable, bd_decomp, "--module", module, "--top", "1", "--json"],
        cwd=_REPO_ROOT, capture_output=True, text=True, timeout=60,
    )
    assert as_json.returncode == 0, as_json.stderr
    import json
    payload = json.loads(as_json.stdout)
    json_gate_ids = [g["gate"] for g in payload["gates"]]
    assert "FG-FUNCTION-INDEX-LINESHIFT" in json_gate_ids, (
        f"--json report() no longer emits the FUNCTION_INDEX gate: {json_gate_ids!r}"
    )
    json_remedy = next(g["do"] for g in payload["gates"]
                        if g["gate"] == "FG-FUNCTION-INDEX-LINESHIFT")
    assert "OWED TO THE INTEGRATOR" in json_remedy, (
        f"--json report()'s FUNCTION_INDEX remedy is not the corrected text: "
        f"{json_remedy!r}"
    )

    selftest = subprocess.run(
        [sys.executable, bd_decomp, "--selftest"],
        cwd=_REPO_ROOT, capture_output=True, text=True, timeout=60,
    )
    assert selftest.returncode == 0, selftest.stdout + selftest.stderr
    assert "SELFTEST PASS" in selftest.stdout, (
        f"--selftest (exercises g_plain/g_route, both gates_for() call sites) "
        f"no longer passes: {selftest.stdout!r}"
    )
