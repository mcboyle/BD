...
def _strip_row402_commitments(backend):
    ...
    backend._data.pop("verifier", None)
    backend._data.pop(
        getattr(backend, "_COMMITMENT_AUTHORITY_FIELD", ""), None
    )
    backend._data["ciphertexts"].pop(
        getattr(backend, "_ROLLBACK_COMMITMENT_KEY", ""), None
    )
