...
Backend checks use a fake venv executable when their behavior is under test.
"""
import hashlib
import os
import shlex
...
def _fake_backend_python(dirpath, expected_dir, exit_code=0):
    """Write a fake venv Python that requires the deploy install directory."""
    ...
    with open(p, "w") as f:
        ...
        f.write('fi\n')
        if exit_code:
            f.write("printf 'private probe stderr' >&2\n")
            f.write('exit %s\n' % exit_code)
        else:
            f.write("printf 'cloakbrowser'\n")
    os.chmod(p, os.stat(p).st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)
    return p
...
def _run(args, binroot, env_extra=None, cwd=None):
    env = dict(os.environ)
    if os.name == "nt":
...
def test_backend_probe_runs_from_install_dir_not_caller_dir():
    work = tempfile.mkdtemp(prefix="bd_dep_")
    instdir = os.path.join(work, "install"); os.makedirs(instdir)
    ...
    zip_path, sha = _make_zip(work)
    _fake_curl(binroot, "3.66.214")
    fake_python = _fake_backend_python(
        os.path.join(work, "venv-bin"), _git_bash_path(instdir))
    r = _run(["--zip", zip_path, "--expect", "3.66.214", "--sha", sha,
              "--dir", instdir, "--health-url", "http://x/api/health",
              "--timeout", "5", "--interval", "1"], binroot,
             {"BD_VENV_PYTHON": _git_bash_path(fake_python)}, cwd=caller_dir)
    assert r.returncode == 0, r.stdout + r.stderr
    assert "resolve_backend()==cloakbrowser confirmed" in r.stdout


def test_backend_probe_failure_has_generic_error_without_probe_stderr():
    work = tempfile.mkdtemp(prefix="bd_dep_")
    instdir = os.path.join(work, "install"); os.makedirs(instdir)
    caller_dir = os.path.join(work, "caller"); os.makedirs(caller_dir)
    binroot = os.path.join(work, "bin")
    ...
    _fake_curl(binroot, "3.66.214")
    fake_python = _fake_backend_python(
        os.path.join(work, "venv-bin"), _git_bash_path(instdir), exit_code=7)
    r = _run(["--zip", zip_path, "--expect", "3.66.214", "--sha", sha,
              "--dir", instdir, "--health-url", "http://x/api/health",
              "--timeout", "5", "--interval", "1"], binroot,
             {"BD_VENV_PYTHON": _git_bash_path(fake_python)}, cwd=caller_dir)
