...
def test_empty_collection_and_envelope_classification_against_declared_taxonomy():
    """Verify empty collections and envelopes classify as EMPTY_COLLECTION against runtime-derived taxonomy."""
    assert dataset_structural_classifier is not None, "capability missing"
    from bulk_downloader.dataset_structural_classifier import (
        DatasetStructuralClassifier,
        DatasetKind,
        get_dataset_classifier_info,
    )

    # Runtime-derived denominator from classifier declared taxonomy
    info = get_dataset_classifier_info()
    declared_taxonomy = set(info["taxonomy"])
    declared_kinds = {k.value for k in DatasetKind}
    assert declared_taxonomy == declared_kinds, "Taxonomy descriptor matches DatasetKind members"
    assert DatasetKind.EMPTY_COLLECTION.value in declared_taxonomy, "EMPTY_COLLECTION in declared taxonomy"

    clf = DatasetStructuralClassifier(min_items=3)

    # 1. Empty paginated envelope: {"page":1,"total_count":0,"items":[]}
    empty_envelope = {"page": 1, "total_count": 0, "items": []}
    res_env = clf.classify_json(empty_envelope)
    assert res_env.ok is True
    assert res_env.kind in DatasetKind, f"Outcome {res_env.kind} not in declared outcome set"
    assert res_env.kind == DatasetKind.EMPTY_COLLECTION
    assert res_env.kind.value in declared_taxonomy
    assert res_env.is_single is False, "Empty collection must not be classified as single entity"
    assert res_env.is_aggregate is False, "Empty collection contains no items to aggregate"
    assert res_env.item_count == 0
    assert res_env.is_empty is True

    # 2. Empty JSON root list: []
    res_list = clf.classify_json([])
    assert res_list.ok is True
    assert res_list.kind == DatasetKind.EMPTY_COLLECTION
    assert res_list.kind.value in declared_taxonomy
    assert res_list.is_single is False, "Empty root list must not be classified as single entity"
    assert res_list.is_aggregate is False
    assert res_list.item_count == 0
    assert res_list.is_empty is True

    # 3. Empty collection items dict without pagination: {"items": []}
    res_items = clf.classify_json({"items": []})
    assert res_items.ok is True
    assert res_items.kind == DatasetKind.EMPTY_COLLECTION
    assert res_items.kind.value in declared_taxonomy
    assert res_items.is_single is False
    assert res_items.is_aggregate is False
    assert res_items.item_count == 0

    # 4. Positive control: envelope with items < threshold but pagination metadata is aggregate
    res_small_agg = clf.classify_json({"page": 1, "total_count": 2, "items": [{"id": 1}, {"id": 2}]})
    assert res_small_agg.ok is True
    assert res_small_agg.kind == DatasetKind.AGGREGATE_DATASET
    assert res_small_agg.is_aggregate is True
    assert res_small_agg.is_single is False
    assert res_small_agg.item_count == 2

    # 5. Positive control: single entity dict
    res_single = clf.classify_json({"id": "asset-1", "title": "Item 1", "url": "https://example.com/1"})
    assert res_single.ok is True
    assert res_single.kind == DatasetKind.SINGLE_ENTITY
    assert res_single.is_single is True
    assert res_single.is_aggregate is False

    # All decisions partition the runtime declared taxonomy denominator
    observed_kinds = {res_env.kind.value, res_list.kind.value, res_small_agg.kind.value, res_single.kind.value}
    assert observed_kinds.issubset(declared_taxonomy)
