...
def test_present_corpus_round_trips_bytes_mtime_and_declared_state(tmp_path: Path) -> None:
    ...
    create = getattr(bk, "create_capture_corpus_backup", None)
    restore = getattr(bk, "restore_capture_corpus_backup", None)
    assert callable(create) and callable(restore), (
        "backup module has no explicit capture-corpus backup/restore contract"
    )
    created = create(archive, source_root=source)
    ...
    restored = restore(archive, target_root=destination)
...
def test_restore_rejects_checksum_corruption_before_creating_target_paths(
        tmp_path: Path) -> None:
    source = tmp_path / "source"
    _make_present(source)
    archive = tmp_path / "corpus.zip"
    created = bk.create_capture_corpus_backup(archive, source_root=source)
    assert created["ok"] is True and created["files"] == len(_output_dirs()), created

    with zipfile.ZipFile(archive) as original:
        names = original.namelist()
        assert len(names) == len(_output_dirs()) + 1
        payloads = {name: original.read(name) for name in names}
    member = "captures/nested/example.wacz"
    assert payloads[member] == b"synthetic-row089-capture"
    payloads[member] = b"corrupted-row089-capture"
    corrupt = tmp_path / "corrupt.zip"
    with zipfile.ZipFile(corrupt, "w") as rewritten:
        for name in names:
            rewritten.writestr(name, payloads[name])

    target = tmp_path / "target"
    restored = bk.restore_capture_corpus_backup(corrupt, target_root=target)

    assert restored["ok"] is False, restored
    assert "checksum mismatch" in restored["error"]
    assert restored["files_restored"] == 0
    assert not target.exists(), "corrupt archive must fail before creating output roots"
