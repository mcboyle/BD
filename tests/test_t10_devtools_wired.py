"""Current template, macros, developer tools, plugins, and i18n SPA contract.

Proves the 16 named endpoint families remain SPA-wired; the three section components carry full /api/ literals
...
macros/replay carries the INV-001 pause-workers warning.
...
def test_no_hook_uses_root_dynamic_api_dispatch():
    """Hooks use named API families rather than unrestricted root dispatch."""
    # The current hooks must NOT root-dispatch via a dynamic /api/${...}.
    # Named endpoints are asserted directly above; there is no historical
    # count/baseline whose magnitude can go stale.
