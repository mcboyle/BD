...
def test_literal_local_equality_is_decided_without_inventing_runtime_values(
        tmp_path):
    """Row 26 slice 5: resolve only literal-local equality/inequality."""
    source = (
        "def test_it(observed):\n"
        "    expected = 12\n"
        "    same = 12\n"
        "    assert expected == 12\n"
        "    assert observed == expected\n"
    ...
    assertions = [node for node in ast.walk(tree)
                  if isinstance(node, ast.Assert)]
    assert len(assignments) == 2 and len(assertions) == 3, (
        "comparison fixture must contain two literal bindings and three asserts")
    assert all(isinstance(node.test, (ast.Compare, ast.BoolOp))
               for node in assertions)

    path = tmp_path / "test_literal_local_equality.py"
    path.write_text(source, encoding="utf-8")
    hits = _vacuous_asserts(path)

    assert hits == [(4, "expected == 12")], (
        "literal-local equality was not found exactly once, or an unresolved/"
        f"false comparison was invented as vacuous: {hits}"
    )
