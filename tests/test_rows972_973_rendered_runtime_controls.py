...
def _settings_center_get_routes(client):
    """Every GET route this blueprint actually registers, taken from the live url_map.

    The denominator is the app, not a hand-kept list: a read route added later joins
    this census automatically instead of quietly escaping it (rule 8).
    """
    paths = []
    for rule in client.application.url_map.iter_rules():
        if rule.endpoint.split(".")[0] != "settings_center":
            continue
        if "GET" not in rule.methods:
            continue
        paths.append(rule.rule.replace("<sid>", "example"))
    assert len(paths) >= 6, f"read-route denominator collapsed: {paths}"
    return sorted(paths)


def test_proxy_credentials_never_appear_on_any_settings_read_route(client):
    """B7-B's HIGH: the write path masked its echo but STORED the raw value, and
    /api/settings/global/effective returns the store through _mask_secrets, where
    _is_secret("BD_HTTP_PROXY") was False -- so the password came back verbatim from
    the same blueprint that had just masked it.

    The denominator here is every GET route the blueprint registers, not the three the
    earlier test named. The positive control is that the probe CAN see the secret: the
    same substring is asserted present in the value the product reader returns, so a
    green result cannot mean "the probe looked at nothing".
    """
    raw = "http://leak_user:leak_pass_7f3a@proxy-leak.invalid:8080"
    written = client.post(WRITE, data={"BD_HTTP_PROXY": raw})
    assert written.status_code in (200, 303), f"credentialed write absent: {written.status_code}"

    # POSITIVE CONTROL: the credential really is in the system under test, so any
    # read route that echoed it would be caught by the substring below.
    assert "leak_pass_7f3a" in http_client._proxy_url()

    leaked = []
    for path in _settings_center_get_routes(client) + [WRITE]:
        reply = client.post(path, data={}) if path == WRITE else client.get(path)
        text = reply.get_data(as_text=True)
        if "leak_pass_7f3a" in text or "leak_user" in text:
            leaked.append(f"{path} -> {reply.status_code}")
    assert leaked == [], "proxy credentials echoed by read route(s): " + ", ".join(leaked)
...
def test_untrimmed_env_value_is_still_masked_before_it_reaches_the_page(client, raw):
    """B11-B F1 (HIGH): _effective_proxy() falls back to the RAW env var, and a single
    leading space made the scheme regex miss, so _split_authority declared the value
    credential-free and the password was rendered verbatim into value='...'.

    The control is the same probe on the trimmed value, asserted below to mask: so a
    green result here cannot mean the probe is blind to the credential.
    """
    client.environment.setenv("BD_HTTP_PROXY", raw)
    rendered = Surface(page(client)).control("BD_HTTP_PROXY")["value"]
    assert "s3cret" not in rendered, f"password rendered from untrimmed value: {rendered!r}"
    assert "***:***@" in rendered, f"userinfo not masked at all: {rendered!r}"
    # positive control: the trimmed form of the same value masks, and the probe sees it.
    client.environment.setenv("BD_HTTP_PROXY", raw.strip())
    assert "***:***@" in Surface(page(client)).control("BD_HTTP_PROXY")["value"]


@pytest.mark.parametrize("hostless", ["http://?x=1", "http://#f", "http://", "https://?a=b#c"])
def test_hostless_proxy_is_refused_however_the_authority_ends(client, hostless):
    """B11-B F2: the host was derived as rest.partition("/")[0], which never sees "?" or
    "#", so "http://?x=1" and "http://#f" were ACCEPTED and persisted -- hostless values
    the docstring says are refused because the product reader would return None.
    """
    ok, resolved, error = sc._validate_proxy(hostless)
    assert ok is False and resolved is None, f"hostless value accepted: {hostless!r}"
    assert "host" in (error or "").lower(), error
    # the write route must refuse it too, not just the helper.
    r = client.post(WRITE, data={"BD_HTTP_PROXY": hostless})
    assert r.status_code == 400, f"{hostless!r} accepted by the write route"
