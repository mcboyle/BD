...
def browser():
    ...
    with sync_playwright() as pw:
        # An absent browser binary is an UNAVAILABLE INSTRUMENT, not a defect in
        # the subject, and CI runners do not ship one (`playwright install` is
        # not run there). Reporting it as FAILURE makes this gate indistinguish-
        # able from a real affordance-learning regression -- the exact inversion
        # CLAUDE.md A7 forbids. Skip with the reason, matching the convention
        # already used in tests/test_element_pick_bridge.py:51.
        try:
