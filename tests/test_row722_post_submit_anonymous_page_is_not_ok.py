...
def test_refused_page_is_kept_as_evidence(monkeypatch, tmp_path):
    ...
    # on the navigated path main reads page.content() twice BEFORE this
    # row's check -- row 772's early rejected-login probe and row 772's
    # settled-page read -- and this row's evidence write is the third.
    # Pinned on the base without row 772 this was 1 (evidence only).
    assert calls["content"] == 3, calls
...
def test_member_affordance_without_password_field_is_still_ok(monkeypatch, tmp_path):
    ...
    # Merged rows 722s+772: row 772's early rejected-login probe and its
    # settled-page read are the only two reads on an accepted navigation;
    # an evidence pass would be a third. Pinned 0 on the base without 772.
    assert calls["content"] == 2, "no evidence pass on an accepted navigation"
    assert list((tmp_path / "evidence").glob("*")) == [], \
        "no evidence is written for an accepted navigation"
