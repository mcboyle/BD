...
def test_the_established_guard_is_built_against_the_real_httpx_under_a_stand_in(monkeypatch, kind):
    ...
    guard_cls = _common._SSRFGuardedTransport_factory()
    assert isinstance(transport, guard_cls)
    assert _common._SSRFGuardedTransport_factory() in type(transport).__mro__
    assert transport._allow_private_hosts is False
    assert _common._SSRFGuardedTransport_factory() is guard_cls
...
@pytest.mark.parametrize("policy_name", ["PINNED", "PUBLIC_ONLY"])
def test_pinning_preserves_cookie_and_redirect_request_identity(monkeypatch, policy_name):
    """The wire may use the vetted literal; httpx must retain the hostname for
    cookies and same-origin redirects."""
    import httpx
    st = _seam()
    calls = []
    ...
    def peer(self, request):
        ...
        if len(calls) == 1:
            return httpx.Response(302, headers={
                "location": "https://fixture.test/next",
                "set-cookie": "SID=zero-entropy; Domain=fixture.test; Path=/"})
        return httpx.Response(200, content=b"ok")

    monkeypatch.setattr(socket, "getaddrinfo", resolve)
    monkeypatch.setattr(httpx.HTTPTransport, "handle_request", peer)
    with httpx.Client(transport=st.guarded_transport(getattr(st, policy_name)),
                      follow_redirects=True,
                      headers={"authorization": "Bearer zero-entropy"}) as client:
        assert client.get("https://fixture.test/start").text == "ok"
    assert calls[0][0].startswith("https://8.8.8.8/")
    assert calls[1] == ("https://8.8.8.8/next", "SID=zero-entropy",
                        "Bearer zero-entropy")


def test_deep_detect_public_client_preserves_cookie_and_redirect_identity(monkeypatch):
    """Production PUBLIC_ONLY constructor keeps its named-cookie jar domain."""
    import httpx
    from bulk_downloader.deep_detect.orchestrate import _build_default_http_client
    calls = []
    ...
    def peer(self, request):
        calls.append((str(request.url), request.headers.get("cookie")))
        if len(calls) == 1:
            return httpx.Response(302, headers={"location": "https://fixture.test/next",
                "set-cookie": "SID=zero-entropy; Domain=fixture.test; Path=/"})
        return httpx.Response(200, content=b"ok")
    monkeypatch.setattr(httpx.HTTPTransport, "handle_request", peer)
    with _build_default_http_client() as client:
        assert client.get("https://fixture.test/start").text == "ok"
    assert calls[1] == ("https://8.8.8.8/next", "SID=zero-entropy")


def test_proxy_transport_delegates_to_the_vetted_literal_before_restoring_identity(monkeypatch):
    """The proxy transport receives the pinned origin, never a re-resolvable name."""
    import httpx
    st = _seam()
    dns, seen = [], []
    ...
    monkeypatch.setattr(httpx.HTTPTransport, "handle_request",
                        lambda self, request: seen.append(str(request.url)) or httpx.Response(204))
    with httpx.Client(transport=st.guarded_transport(st.PINNED, proxy="http://127.0.0.1:1")) as client:
        assert client.get("https://fixture.test/path").status_code == 204
    assert dns == ["fixture.test"]
    assert seen == ["https://203.0.113.5/path"]


def test_public_proxy_transport_refuses_a_private_origin_before_delegating(monkeypatch):
    """PUBLIC_ONLY keeps its all-address public classification through a proxy."""
    import httpx
    ...
    with httpx.Client(transport=st.guarded_transport(
            st.PUBLIC_ONLY, proxy="http://127.0.0.1:1")) as client:
        with pytest.raises(SSRFBlocked, match="refusing private"):
            client.get("https://private-origin.test/path")
    assert seen == []
