...
def _own_the_staging_path(dest, page_url):
    """Mint the PROVEN staging claim an interrupted download would hold.

    WHY THIS EXISTS, AND WHY DROPPING A ``.part`` ON DISK IS NO LONGER ENOUGH.
    Rows 481/489/492/533/535/541 landed after this work was first written:
    ``staging_claim.claim`` now measures the provenance of any bytes already at
    the staging path and, when no completed claim names them, renames them to
    ``.orphaned-*.part`` -- because an ownerless partial is some other job's
    abandoned bytes, and adopting them is the 2026-08-29 corruption.

    So a fixture that merely writes a ``.part`` no longer describes a resumable
    download at all: the transport sets those bytes aside, ``resume_from`` is 0,
    no ``Range`` goes out, and every assertion about the resume becomes vacuous
    rather than false.  Minting the claim BEFORE the bytes are written (the
    order production uses) makes the ``.part`` this job's own partial, which is
    the only state in which rows 428 and 430 are reachable.

    Returns the staging path, which is also the assertion that it is the one
    the transport will use.
    """
    from bulk_downloader import staging_claim
    identity = staging_claim.job_identity(page_url)
    staged = staging_claim.claim(dest, identity)
    assert staged == staging_claim.staging_path_for(dest), staged
    return staged


def _prepare_part(tmp_path, name, part_bytes, meta=None, *, page_url):
    dest = tmp_path / name
    part = _own_the_staging_path(dest, page_url)
    assert part == dest.with_suffix(dest.suffix + ".part"), part
    ...
    # And the precondition the claim exists for: re-claiming under the same
    # identity -- which is exactly what the transport is about to do -- must
    # RESUME these bytes, not orphan them.  Asserted behaviourally rather than
    # by reading the claim record, so it measures what the transport measures.
    assert _own_the_staging_path(dest, page_url) == part
    assert part.stat().st_size == len(part_bytes), (
        "re-claiming set the .part aside, so this fixture is not describing a "
        "resumable download")
    orphans = sorted(p.name for p in tmp_path.glob("*.orphaned-*"))
...
def test_a_kill_at_a_checkpoint_resumes_without_holes(frozen_run, monkeypatch,
    ...
    room.mkdir()
    dest = room / "par.bin"
    # A SIGKILL leaves the staging CLAIM behind as well as the bytes -- it is
    # a file on the same disk -- and since row 481 the claim is what makes
    # those bytes resumable rather than someone else's orphan.  Restoring only
    # the bytes would have the transport set them aside and restart from 0,
    # which passes no assertion here and proves nothing about the checkpoint.
    part = _own_the_staging_path(dest, str(dest))
...
def test_an_unmeasurable_part_refuses_reconciliation(monkeypatch, tmp_path):
    """Row 431's A7 clause, MEASURED on today's tree rather than assumed.

    The row's acceptance also asks that ``resume.reconcile_with_disk`` on an
    unmeasurable ``.part`` stay UNKNOWN/refuse rather than OK.  THIS CUT DOES
    NOT CHANGE THAT PATH -- ``origin/main`` already satisfies the clause, and
    this is the control that says so and keeps it said:

      * a MISSING ``.part`` returns False, already gated by
        ``test_v3_43_27_resume.py::test_reconcile_with_disk_rejects_missing_part``;
      * an UNREADABLE ``.part`` refuses by propagating the OSError out of
        ``pathlib.Path.exists``, which does not swallow EIO.  That is a
        refusal, and the LOUDER of the two available ones: the function's own
        ``except OSError: return False`` would fold an unreadable file into the
        caller's "validators do not match; starting fresh" branch, which is a
        different diagnosis leading to a different action (CLAUDE.md A7 on
        collapsed diagnostics).  So the behaviour is pinned as it is rather
        than normalised into a bool.

    Either way the one outcome that must never occur is a True: that would
    resume over bytes whose length was never read.
    """
    from bulk_downloader import resume as _resume

    dest = tmp_path / "unmeasurable.bin"
    part = _resume.part_path(dest)
    part.write_bytes(b"\x00" * 4096)
    cp = {"total": 4096, "chunks": [{"start": 0, "end": 4095, "done_bytes": 0}]}
    # PRECONDITION: while the .part IS measurable this checkpoint reconciles,
    # so a False/raise below is caused by the injected failure and nothing else.
    assert _resume.reconcile_with_disk(cp, dest) is True

    real_stat = Path.stat
    fired = {"n": 0}

    def unreadable(self, *a, **kw):
        if str(self) == str(part):
            fired["n"] += 1
            raise OSError(5, "simulated unreadable .part")
        return real_stat(self, *a, **kw)

    monkeypatch.setattr(Path, "stat", unreadable)
    try:
        outcome = ("returned", _resume.reconcile_with_disk(cp, dest))
    except OSError as e:
        outcome = ("raised", e)
    ...
    assert outcome != ("returned", True), (
        "an unmeasurable .part reconciled as OK: the resume would then trust a "
        "checkpoint over bytes whose length it could not read")
