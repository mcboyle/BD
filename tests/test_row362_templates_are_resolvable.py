...
def test_negative_control_known_to_match_does_not_report_miss():
    ...
    assert independent_values.count(_SAFE_ROWS) == 1, (
        "precondition: committed gamma template lacks the fixture's row selector"
    )
    soup = _soup(_PRESENT)
    assert len(soup.select(_SAFE_ROWS)) == 8, (
        "precondition: negative-control fixture no longer has eight known hits"
    )

    report = _api().verify_template_source("gamma_kosmos", _PRESENT)

    row = _entry(report, _SAFE_ROWS)
    assert row["status"] != "MISS", row
    assert row["status"] == "HIT" and row["count"] == 8, row
...
def test_gamma_login_uses_the_measured_input_submit_control():
    ...
    assert values.count("#submit") == 1, (
        "precondition: gamma template must carry the observed submit selector"
    )
    old_guesses = [
...
def test_installed_cli_resolves_the_checkout_pointer(tmp_path: Path):
    ...
    run = subprocess.run(
        [str(bare_python), str(tool), "--selftest"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
        timeout=30,
    ...
    assert run.returncode == 0, run.stdout + run.stderr
    assert "SELFTEST PASS" in run.stdout, run.stdout
