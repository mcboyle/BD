...
def _mutation_evidence_auditor():
    ...
    return namespace["_audit_release_mutation_evidence"]
...
def test_every_later_quantitative_receipt_carries_its_durable_evidence():
    """Mandatory only inside the independently decidable receipt population."""
    revisions = _changelog_revisions_since_adoption()
    omission_index = revisions.index(_LIVE_RECEIPT_WITHOUT_SPEC)
    ...
    enforced = records[omission_index + 1:]
    failures = [record for record in enforced if record["problems"]]
