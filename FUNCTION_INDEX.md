...
- L0567 `_token_eq` `[private]` — AF1 (v3.66.41): constant-time comparison for auth secrets. Plain
- L0579 `_check_token` `[private]`
- L0770 `_session_valid` `[private]` — True iff the session exists and hasn't expired (idle TTL).
- L0782 `_session_touch` `[private]` — Slide the idle TTL by updating last_used.
- L0789 `_session_create` `[private]` — Mint a new session. source is for diagnostics — "pair_redeem",
- L0799 `_csrf_token_for` `[private]` — Derive a CSRF token from the session token. Deterministic per
- L0844 `csrf_fires_for` — Route-level answer to: would `_check_csrf` 403 a cookie-session browser
- L0864 `_check_csrf` `[private]` — For state-changing requests (POST/PUT/PATCH/DELETE) under /api/,
- L0962 `_add_security_headers` `[private]` — Audit 2026-05 / Phase 2C: defense-in-depth response headers.
- L1016 `_dev_metrics_start` `[private]`
- L1024 `_dev_metrics_record` `[private]`
- L1107 `_bootstrap_session` `[private]` — If the request is for the HTML shell (GET /) and there's no
- L1169 `serve_mobile_view` `GET /m/; GET /m`
- L1186 `serve_mobile_ops_view` `GET /m/ops/; GET /m/ops`
- L1217 `_m2_503_node_missing` `[private]` — Uniform 503 response when frontend/dist/ is missing.
- L1272 `_m2_opt_state` `[private]` — Return one of 'v2', 'v1', or 'default' based on query + cookie.
- L1288 `_m2_apply_opt_cookie` `[private]` — If the request had `?ui=v2` or `?ui=v1`, persist that as the
- L1307 `serve_m2_spa` `GET /m2/; GET /m2; GET /m2/<path:subpath>` — Phase 1 root flip (v3.66.203): the D3 SPA moved from /m2 to `/`
- L1354 `_resolve_sites_file` `[private]` — Where the site config lives.
- L1394 `_save_sites_config` `[private]` — Write current sites to disk. Called on every add/update/delete.
- L1471 `_build_meta` `[private]` — Return a copy of cfg with secrets stripped — top-level password
- L1510 `_load_sites_config` `[private]` — Read sites from disk on startup and instantiate SiteRunners.
- L1664 `_start_session_keepers` `[private]`
- L1735 `_heartbeat_to_disk_loop` `[private]`
- L1775 `_start_watch_folder_threads` `[private]` — Spawn one daemon thread per configured site. The thread's
- L1809 `_start_window_scheduler` `[private]`
- L1878 `_start_storage_tier_scheduler` `[private]`
- L1897 `_load_app_config` `[private]`
- L1990 `_save_app_config` `[private]` — Persist global app config. Atomic via .tmp + replace (v3.43.19):
- L2052 `_start_background_services` `[private]` — Start bg_scheduler + the webhook drain worker. Called from boot_once().
- L2134 `_route_urls_internal` `[private]` — Internal helper: same routing logic as /api/route_urls but without
- L2167 `_saved_search_enqueue_handler` `[private]`
- L2182 `_score_url_against_sites` `[private]` — v3.43.40: extracted scoring helper used by both routing
- L2274 `_watcher_loop` `[private]` — Thread body. Polls watch_folder for new .txt files; imports URLs;
- L2344 `_start_watcher` `[private]` — Idempotent start. Called at module import + whenever app_config
- L2401 `_explicit_methods_for_path` `[private]` — Methods served by EXPLICIT rules (anything but the serve_spa_root
- L2419 `_405_with_allow` `[private]`
- L2427 `_method_not_allowed_parity` `[private]` — Restore pre-flip 404/405 semantics for non-GET requests whose
- L2460 `serve_spa_root` `GET /; GET /<path:subpath>` — Serve the D3 React SPA from frontend/dist/ at the site root.
- L2543 `pwa_manifest` `GET /manifest.json`
- L2559 `pwa_icon_svg` `GET /icon.svg` — Same icon as the favicon — bundled inline so we don't need a static dir.
- L2672 `pwa_sw` `GET /sw.js`
- L2692 `_origins_env_locked` `[private]` — A field is env-locked when a BD_<UPPER(field)> env var pins it. Static
- L2704 `_oi_flagged` `[private]` — Best-effort count of sites whose status looks problematic. Fail-soft:
- L2730 `_oi_dir_writable` `[private]` — (exists, writable) for a candidate dir — read-only, never creates.
- L2739 `_oi_default_download_dir` `[private]` — The directory a site downloads into when it names none of its own.
- L2764 `_chk` `[private]`
- L2885 `_request_is_same_origin` `[private]` — True when the request's Referer host:port matches its Host -- i.e. it
- L2908 `_dev_request_authorized` `[private]` — F-APP04-01: the /api/dev/* surface is privileged (lint / probe /
- L2939 `_dev_mode_guard` `[private]` — Return a 404 response if dev mode is off, or a 403 if the request is not
- L3420 `metrics_endpoint` `GET /metrics` — Prometheus text-format exposition. Scrape-friendly; no auth.
- L3455 `_bd_cookie_dir` `[private]`
- L3462 `_site_primary_url` `[private]` — Resolve a site's primary URL from the usual config fields (mirrors
- L3518 `stream_serve` `GET /stream/<token>` — Serve a video file with HTTP range support, gated by token.
- L3604 `_capture_enqueue` `[private]` — Inject seam for capture_schedules.run_*: append URL(s) to a site's
- L3721 `_m2_avatar_color` `[private]` — Deterministic name → color (one of 12 hues). Same input always
- L3734 `_m2_site_drain_eta` `[private]` — F1.6: estimate seconds to drain one site's queue from that site's
- L3752 `_m2_auth_state` `[private]` — Bucket the runner's auth state into ok/expired/unknown.
- L3800 `_m2_attention_for_site` `[private]` — Return an attention-banner entry for a site, or None if it has
- L3850 `_m2_age_human` `[private]` — Compact human age — '2h ago', '15m ago', '3d ago'. Empty if
- L3869 `_m2_honeypot_suggestion` `[private]` — Advisory per-site honeypot drop-threshold suggestion for the
- L3896 `_m2_activity_query_fragments` `[private]` — Build (where_clauses, params) for activity_v2 + export endpoints.
- L3987 `_diff_parse_target` `[private]` — Parse a colon-separated 'site_id:url' from the query string.
- L4003 `_diff_collect_one` `[private]` — Resolve one diff side. Returns a dict with keys site_id, url,
- L4045 `_diff_lines_for` `[private]` — Render an events list as one string per event, formatted
- L4104 `_status_snapshot` `[private]` — Build the same dict shape that /api/status would return. Extracted
- L4136 `_dashboard_snapshot` `[private]` — Build the same dict that /api/dashboard returns.
- L4266 `_validate_path` `[private]` — Returns (ok: bool, normalized_path_or_error_message: str).
- L4311 `_reveal_safe_roots` `[private]` — F-APP06-01: the effective allowlist for the reveal action -- the
- L4346 `_validate_reveal_path` `[private]` — F-APP06-01: reveal-scoped path check. Runs the standard _validate_path
- L4371 `_validate_config_paths` `[private]` — Run _validate_path on every path-bearing field. Returns
- L4405 `_sanitize_display_name` `[private]` — Normalize a user-facing display string. Returns the cleaned value.
- L4425 `_create_site` `[private]` — Create one site from a config dict. Returns (sid, error).
- L4507 `_apply_template_by_id` `[private]` — Merge a template's learned block + config_defaults into a site,
- L4536 `_apply_login_template_by_id` `[private]` — Merge a LOGIN template's selectors into a site's learned.login.
- L4564 `_apply_detected_selectors` `[private]` — v3.66.0: merge selectors discovered by auto_detect.detect_site_config
- L4614 `_auto_pick_templates` `[private]` — v3.65.2: Automatically apply matching login + download templates
- L4811 `_vault_guard_for_password` `[private]` — v3.66.326: gate storing a site login password in the secrets vault.
- L4847 `_store_site_password_in_vault` `[private]` — v3.66.326: store ``password`` for ``sid`` in the secrets vault and
- L5008 `_lan_ip_guess` `[private]` — Best-effort detection of this host's LAN IP. Uses the "connect
- L5072 `_teach_cors_response` `[private]` — Add CORS headers for the takeover browser. The teach overlay
- L5212 `_require_vault_token` `[private]` — Helper that validates the Authorization: Bearer <vault_token>
- L5229 `_reject_if_vault_token` `[private]` — B12 (v3.66.38): management routes (pair_issue / list_paired /
- L5421 `_rate_sweep_locked` `[private]` — Drop bucket entries with no timestamps newer than the window. Caller
- L5434 `_is_url_public` `[private]` — AUDIT FIX (v3.43.16): SSRF defence. Resolve the hostname and ensure
- L5461 `_rate_check` `[private]` — Return True if the request should be allowed, False if rate
- L5481 `_do_action` `[private]` — Common body for start/pause/resume/stop/clear/retry. Rate-limits
- L5521 `_do_action_all` `[private]` — Apply `action` to every runner. Returns aggregate result.
- L5589 `_validate_bulk_urls` `[private]` — Common URL list validation. Returns (ok, urls_or_error_dict).
- L5731 `serve_ss` `GET /screenshots/<path:filename>`
- L7010 `_global_notify_settings_path` `[private]` — Where the GLOBAL apprise settings live (not per-site).
- L7017 `_load_global_notify_settings` `[private]` — Load global apprise settings from disk. Fail-open.
- L7028 `_save_global_notify_settings` `[private]` — Persist global apprise settings. Fail-open. Atomic write so a
- L7043 `_apply_global_notify_config` `[private]` — Push the saved settings into the dispatcher singleton.
- L7107 `_tg_get_status` `[private]` — Callback for /status: build the site overview dict.
- L7136 `_tg_get_queue` `[private]` — Callback for /queue: return the queue for a site.
- L7144 `_tg_add_url` `[private]` — Callback for /mirror: auto-route and add.
- L7161 `_tg_cancel_url` `[private]` — Callback for /cancel: find the matching pending job and mark it
- L7173 `_tg_retry_site` `[private]` — Callback for /retry: reset failed→pending in one site or all.
- L7196 `_tg_pause_site` `[private]` — Callback for /pause: pause one site or all.
- L7212 `_tg_resume_site` `[private]` — Callback for /resume: resume one site or all.
- L7228 `_persist_cfg` `[private]` — Helper to save s_cfg back to disk.
- L7245 `_apply_tg_bot_config` `[private]` — Push saved settings into the bot singleton.
- L7298 `_dedup_get_registry` `[private]` — Get/create the singleton registry. Picks DB path from any site's
- L7419 `_serialize_search_result` `[private]` — Convert a SearchResult dataclass to a JSON-friendly dict.
```
...
- L0515 `SiteRunner` `[class]`
  ...
  - L0729 `SiteRunner._scrape_listing_urls` `[private]` — Phase 73: same scrape logic as /api/scrape_listing endpoint —
  - L0821 `SiteRunner.update_config` — Swap in a new config dict and restart the scheduler so the
  - L0829 `SiteRunner.set_cookies_from_file` — Load Playwright-format cookies from `path` into this runner.
  - L0842 `SiteRunner.set_cookies` — Replace the runner's cookie list in memory and bump the
  - L0851 `SiteRunner.cookie_info` — Return a snapshot dict describing cookie health for the UI:
  - L0889 `SiteRunner.start`
  - L0941 `SiteRunner._start_serialized` `[private]`
  - L1241 `SiteRunner._publish_watchdog_snapshot` `[private]` — Publish only a still-current heartbeat snapshot for this run.
  - L1251 `SiteRunner._watchdog_loop` `[private]` — v3.43.24: monitor worker heartbeats. Threads should stamp
  - L1312 `SiteRunner._effective_concurrency` `[private]` — Phase 64 (v3.41.0): bandwidth-aware concurrency. If
  - L1390 `SiteRunner.pause` — Pause the worker pool. Workers finish the URL they're currently
  - L1405 `SiteRunner.resume` — Resume from paused / paused_no_button / low_disk states.
  - L1416 `SiteRunner.stop`
  - L1516 `SiteRunner._current_throughput_bps` `[private]` — Sum recent byte rates for jobs that are still running.
  - L1540 `SiteRunner.get_status` — Return runner state. With `light=True`, omit `jobs` and
  - L1642 `SiteRunner._learned_summary` `[private]` — Compact summary for the UI: which kinds are learned, how many
  - L1659 `SiteRunner.state`
  - L1661 `SiteRunner._compute_site_usage` `[private]` — Phase 65 (v3.38.x): sum the byte size of all files under
  - L1697 `SiteRunner._worker_write_generation` `[private]` — Return a worker-thread generation, or None for control-plane writes.
  - L1704 `SiteRunner._worker_write_generation_is_current` `[private]` — Reject mutations from worker threads whose run was invalidated.
  - L1711 `SiteRunner._update_job` `[private]` — Serialize worker-originated publication against stop/start.
  - L1726 `SiteRunner._update_job_current` `[private]` — Central state-mutation: change a job's status/message, log
  - L2243 `SiteRunner._wait_for_lazy_video` `[private]` — v3.43.75: wait for a <video> or <source> to appear in the
  - L2272 `SiteRunner._playlist_expand_one` `[private]` — v3.43.75: expand one listing URL into scene URLs.
  - L2319 `SiteRunner._search_site` `[private]` — v3.43.77: search this site for `query`. Returns SearchResult.
  - L2377 `SiteRunner._worker_generation_is_current` `[private]`
  - L2381 `SiteRunner._watch_done` `[private]` — Background overseer thread spawned by start(). Polls the queue
  - L2442 `SiteRunner._finalize_watch_done` `[private]` — Commit retry/final state only if this overseer still owns the run.
  - L2502 `SiteRunner._claim_completion_notification` `[private]` — Atomically claim a still-current completion token for delivery.
  - L2534 `SiteRunner._notify_watch_done_if_current` `[private]` — Deliver a completion token only after an atomic current-state claim.
  - L2565 `SiteRunner._requeue_generation_item` `[private]` — Restore eligible work using the documented lifecycle lock order.
  - L2585 `SiteRunner._generation_item_is_processable` `[private]` — Validate a dequeued item against the current run and job state.
  - L2594 `SiteRunner._claim_worker_item` `[private]` — Atomically claim eligible current-run work immediately pre-process.
  - L2619 `SiteRunner._process_worker_url` `[private]` — Claim, map, and process one URL with an unambiguous result.
  - L2640 `SiteRunner._resource_admission_hold` `[private]` — Return a visible hold when a configured resource gate is not safe.
  - L2722 `SiteRunner._worker_loop` `[private]` — One persistent worker thread. Owns its own playwright + browser
  - L3064 `SiteRunner._process_one` `[private]` — Process a single URL.
```
