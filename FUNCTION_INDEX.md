...
- L1978 `_heartbeat_to_disk_loop` `[private]`
- L2018 `_start_watch_folder_threads` `[private]` — Spawn one daemon thread per configured site. The thread's
- L2103 `_resolved_site_runtime_path` `[private]` — Return the absolute identity of the configured-sites runtime.
- L2112 `_require_compatible_site_runtime_path` `[private]` — Refuse to bind the process's live runners to a second config file.
- L2125 `_retire_configured_runtime_runners` `[private]` — Retire every partially loaded runner without losing any handle.
- L2151 `_retire_configured_runtime_watchers` `[private]` — Signal and prove every configured watch generation quiescent.
- L2215 `_retire_configured_runtime_dependencies` `[private]` — Quiesce watcher/keeper producers without resetting VPN leaves.
- L2246 `_retire_configured_runtime_all` `[private]` — Retire all producers, then and only then reset their VPN leaf.
- L2260 `_clear_configured_runtime_state` `[private]` — Clear a partial load only after every owned generation is dead.
- L2277 `_activate_configured_runtime_once` `[private]` — Restore configured sites and their dependent services exactly once.
- L2359 `_start_window_scheduler` `[private]`
- L2428 `_start_storage_tier_scheduler` `[private]`
- L2447 `_load_app_config` `[private]`
- L2540 `_save_app_config` `[private]` — Persist global app config. Atomic via .tmp + replace (v3.43.19):
- L2602 `_start_background_services` `[private]` — Start bg_scheduler + the webhook drain worker. Called from boot_once().
- L2688 `_routing_config_projection` `[private]` — Return the immutable-by-value inputs used for URL/site scoring.
- L2699 `_route_urls_internal` `[private]` — Internal helper: same routing logic as /api/route_urls but without
- L2762 `_saved_search_enqueue_handler` `[private]`
- L2777 `_score_url_against_sites` `[private]` — v3.43.40: extracted scoring helper used by both routing
- L2869 `_watcher_loop` `[private]` — Thread body. Polls watch_folder for new .txt files; imports URLs;
- L2940 `_start_watcher` `[private]` — Idempotent start. Called at explicit boot + whenever app_config
- L2982 `_stop_watcher` `[private]` — Signal and boundedly prove the process-global watcher generation.
- L3054 `_explicit_methods_for_path` `[private]` — Methods served by EXPLICIT rules (anything but the serve_spa_root
- L3072 `_405_with_allow` `[private]`
- L3080 `_method_not_allowed_parity` `[private]` — Restore pre-flip 404/405 semantics for non-GET requests whose
- L3113 `serve_spa_root` `GET /; GET /<path:subpath>` — Serve the D3 React SPA from frontend/dist/ at the site root.
- L3196 `pwa_manifest` `GET /manifest.json`
- L3212 `pwa_icon_svg` `GET /icon.svg` — Same icon as the favicon — bundled inline so we don't need a static dir.
- L3325 `pwa_sw` `GET /sw.js`
- L3345 `_origins_env_locked` `[private]` — A field is env-locked when a BD_<UPPER(field)> env var pins it. Static
- L3357 `_oi_flagged` `[private]` — Best-effort count of sites whose status looks problematic. Fail-soft:
- L3383 `_oi_dir_writable` `[private]` — (exists, writable) for a candidate dir — read-only, never creates.
- L3392 `_oi_default_download_dir` `[private]` — The directory a site downloads into when it names none of its own.
- L3417 `_chk` `[private]`
- L3538 `_request_is_same_origin` `[private]` — True when the request's Referer host:port matches its Host -- i.e. it
- L3561 `_dev_request_authorized` `[private]` — F-APP04-01: the /api/dev/* surface is privileged (lint / probe /
...
- L5248 `_apply_login_template_by_id` `[private]` — Merge a LOGIN template's selectors into a site's learned.login.
- L5276 `_apply_detected_selectors` `[private]` — v3.66.0: merge selectors discovered by auto_detect.detect_site_config
- L5326 `_auto_pick_templates` `[private]` — v3.65.2: Automatically apply matching login + download templates
- L5523 `_vault_guard_for_password` `[private]` — v3.66.326: gate storing a site login password in the secrets vault.
- L5559 `_store_site_password_in_vault` `[private]` — v3.66.326: store ``password`` for ``sid`` in the secrets vault and
- L5720 `_lan_ip_guess` `[private]` — Best-effort detection of this host's LAN IP. Uses the "connect
- L5784 `_teach_cors_response` `[private]` — Add CORS headers for the takeover browser. The teach overlay
- L5924 `_require_vault_token` `[private]` — Helper that validates the Authorization: Bearer <vault_token>
- L5949 `_reject_if_vault_token` `[private]` — B12 (v3.66.38): management routes (pair_issue / list_paired /
- L6153 `_rate_sweep_locked` `[private]` — Drop bucket entries with no timestamps newer than the window. Caller
- L6166 `_is_url_public` `[private]` — AUDIT FIX (v3.43.16): SSRF defence. Resolve the hostname and ensure
- L6193 `_rate_check` `[private]` — Return True if the request should be allowed, False if rate
- L6213 `_do_action` `[private]` — Common body for start/pause/resume/stop/clear/retry. Rate-limits
...
- L0982 `SiteRunner` `[class]`
  ...
  - L1227 `SiteRunner._scrape_listing_urls` `[private]` — Phase 73: same scrape logic as /api/scrape_listing endpoint —
  - L1320 `SiteRunner.update_config` — Swap in a new config dict and restart the scheduler so the
  - L1328 `SiteRunner.set_cookies_from_file` — Load Playwright-format cookies from `path` into this runner.
  - L1341 `SiteRunner.set_cookies` — Replace the runner's cookie list in memory and bump the
  - L1350 `SiteRunner._refresh_worker_cookies` `[private]` — Inject freshly published cookies into a worker's persistent context.
  - L1381 `SiteRunner.cookie_info` — Return a snapshot dict describing cookie health for the UI:
  - L1419 `SiteRunner._begin_auxiliary_start` `[private]` — Register the current auth/manual launcher before it can block.
  - L1429 `SiteRunner._end_auxiliary_start` `[private]` — Release a launcher, retaining its handle after retirement.
  - L1444 `SiteRunner._start_owned_auxiliary_thread` `[private]` — Atomically publish/start one auxiliary generation or refuse it.
  - L1458 `SiteRunner._start_tracked_auxiliary_thread` `[private]` — Publish a callback-only thread in the shared auxiliary registry.
  - L1472 `SiteRunner._finish_tracked_auxiliary_thread` `[private]`
  - L1477 `SiteRunner.start`
  - L1533 `SiteRunner._refuse_for_download_hold` `[private]` — Publish the runner-visible refusal for a held/unmeasurable hold.
  - L1555 `SiteRunner._start_serialized` `[private]`
  - L1920 `SiteRunner._publish_watchdog_snapshot` `[private]` — Publish only a still-current heartbeat snapshot for this run.
  - L1930 `SiteRunner._watchdog_loop` `[private]` — v3.43.24: monitor worker heartbeats. Threads should stamp
  - L1991 `SiteRunner._effective_concurrency` `[private]` — Phase 64 (v3.41.0): bandwidth-aware concurrency. If
  - L2069 `SiteRunner.pause` — Pause the worker pool. Workers finish the URL they're currently
  - L2089 `SiteRunner.resume` — Resume from paused / paused_no_button / low_disk states.
  - L2131 `SiteRunner.stop`
  - L2231 `SiteRunner.retire_workers` — Permanently stop and prove every runner-owned writer quiescent.
  - L2554 `SiteRunner._current_throughput_bps` `[private]` — Sum recent byte rates for jobs that are still running.
  - L2578 `SiteRunner.get_status` — Return runner state. With `light=True`, omit `jobs` and
  - L2680 `SiteRunner._learned_summary` `[private]` — Compact summary for the UI: which kinds are learned, how many
  - L2697 `SiteRunner.state`
  - L2699 `SiteRunner._compute_site_usage` `[private]` — Phase 65 (v3.38.x): sum the byte size of all files under
  - L2735 `SiteRunner._worker_write_generation` `[private]` — Return a worker-thread generation, or None for control-plane writes.
  - L2742 `SiteRunner._worker_write_generation_is_current` `[private]` — Reject mutations from worker threads whose run was invalidated.
  - L2749 `SiteRunner._capture_website_title` `[private]` — Harvest a settled detail page once and retain its provenance.
  - L2815 `SiteRunner._history_title_fields` `[private]` — Return db_log kwargs without inventing a title from a filename.
  - L2846 `SiteRunner._update_job` `[private]` — Serialize worker-originated publication against stop/start.
  - L2867 `SiteRunner._update_job_current` `[private]` — Central state-mutation: change a job's status/message, log
  - L3384 `SiteRunner._wait_for_lazy_video` `[private]` — v3.43.75: wait for a <video> or <source> to appear in the
  - L3413 `SiteRunner._playlist_expand_one` `[private]` — v3.43.75: expand one listing URL into scene URLs.
  - L3464 `SiteRunner._search_site` `[private]` — v3.43.77: search this site for `query`. Returns SearchResult.
  - L3522 `SiteRunner._worker_generation_is_current` `[private]`
  - L3526 `SiteRunner._watch_done` `[private]` — Background overseer thread spawned by start(). Polls the queue
  - L3587 `SiteRunner._finalize_watch_done` `[private]` — Commit retry/final state only if this overseer still owns the run.
  - L3647 `SiteRunner._claim_completion_notification` `[private]` — Atomically claim a still-current completion token for delivery.
  - L3679 `SiteRunner._notify_watch_done_if_current` `[private]` — Deliver a completion token only after an atomic current-state claim.
...
- L0066 `_ManualDownloadSession` `[private]` — Owns a Playwright session for the duration of a manual download
  ...
  - L0484 `_ManualDownloadSession._send` `[private]` — Dispatch a command and wait for the response. Returns the
  - L0501 `_ManualDownloadSession.finalize` — Harvest recordings + cookies, close session. Returns
  - L0511 `_ManualDownloadSession.verify` — Test the user's picked selectors against the live DOM.
  - L0519 `_ManualDownloadSession.test_download` — v3.43.0: dry-run a real fetch of the URL the picks would
  - L0530 `_ManualDownloadSession.commit` — Snapshot cookies and close the session for a Teach Mode commit.
  - L0539 `_ManualDownloadSession.cancel` — Signal closure and prove it within one shared timeout.
  - L0548 `_ManualDownloadSession.snapshot_cookies` — Read cookies without closing the session.
- L0555 `ManualMixin` `[class]`
  - L0557 `ManualMixin.start_manual_download` — Phase 41.3: open a non-headless Chromium at `target_url` with
  - L0595 `ManualMixin.finish_manual_download` — User clicked 'Done' on the download takeover. Harvest the
  - L0677 `ManualMixin.cancel_manual_download` — User clicked Cancel. Close the session, no learning.
  - L0703 `ManualMixin.is_awaiting_manual_download`
```
...
## `bulk_downloader/runner_browser.py` (13 entries)
...
- L0280 `ExtractorsMixin` `[class]`
  ...
  - L0860 `ExtractorsMixin._persist_deep_detect_selectors` `[private]` — Merge deep_detect-discovered selectors into the site's
  - L0896 `ExtractorsMixin._try_jsonapi_extractor` `[private]` — v3.43.68: extract via HereSphere/DeoVR JSON API and download.
  - L1150 `ExtractorsMixin._try_spa_api_media_extractor` `[private]` — Row 722 (G5): API/media extraction fallback for SPA scene pages.
  - L1337 `ExtractorsMixin._try_vixen_extractor` `[private]` — v3.43.67: extract via Vixen __NEXT_DATA__ / <video src> and
  - L1583 `ExtractorsMixin._try_dl8_extractor` `[private]` — v3.43.69: parse <dl8-video> and download.
  - L1806 `ExtractorsMixin._try_aylo_extractor` `[private]` — v3.43.66: extract via Aylo flashvars and download.
  - L2074 `ExtractorsMixin._probe_for_higher_tier` `[private]` — v3.43.65: speculatively probe higher-tier variants of `url`.
  - L2170 `ExtractorsMixin._run_pre_scrape_action` `[private]` — v3.43.65: run a per-site action BEFORE scraping the <video>
  - L2236 `ExtractorsMixin._try_plugin_extractor` `[private]` — PLUGIN-DISPATCH (v3.66.691): run a registered plugin ``@extractor``
  - L2371 `ExtractorsMixin._try_library_extractor` `[private]` — v3.43.63: attempt a library-extractor download for `url`.
```
...
## `bulk_downloader/runner_transport.py` (56 entries)
...
- L0757 `TransportMixin` `[class]`
  ...
  - L1369 `TransportMixin._direct_media_route` `[private]` — (media_url, destination_name) if `href` IS the file, else (None, None).
  - L1442 `TransportMixin._stream_route` `[private]` — (manifest_url, destination_name) if `href` is a stream, else (None, None).
  - L1497 `TransportMixin._probe_outcome` `[private]` — BP-VH1: map a probe result to one of done | streaming | non_media | fail.
  - L1522 `TransportMixin._integrity_size_ok` `[private]` — BP-INT (v3.66.284): True if the received byte count satisfies the
  - L1532 `TransportMixin._promote_or_abort` `[private]` — BP-INT (v3.66.284): atomically promote the ``.part`` to its final
  - L1561 `TransportMixin._do_probe_fetch` `[private]` — GCW probe mode (v3.66.274): the trigger has fired and ``dl.url`` is
  - L1685 `TransportMixin._do_download` `[private]` — Click the download button and save the file. Tries the HTTP path
  - L2583 `TransportMixin._http_download` `[private]` — Run one HTTP transfer inside the RAM staging ownership scope.
  - L2639 `TransportMixin._http_download_claimed` `[private]` — Stream the file URL to disk via httpx, with progress updates,
  - L3193 `TransportMixin._probe_size` `[private]` — HEAD request to learn Content-Length + Accept-Ranges. Returns
  - L3243 `TransportMixin._http_download_parallel` `[private]` — Download `total` bytes via N parallel HTTP Range requests.
  - L3647 `TransportMixin._current_cap_mbps` `[private]` — Return the current effective speed cap in MB/s.
  - L3680 `TransportMixin._recommended_chunk_bytes` `[private]` — Return a chunk size in bytes, tuned to recent observed throughput.
  - L3704 `TransportMixin._observe_throughput` `[private]` — Update the EWMA throughput tracker after a download. Called
```
...
- L2842 `session_event_recent` — Return recent session_history rows. Used by the UI event log.
- L2855 `session_lifetime_observations` — For a given (site, account), find all session lifetimes we've
- L2903 `db_session_failure_clusters` — F2.1: cluster session_history failure events by (site, event_type)
- L2997 `_integrity_state_path` `[private]` — Where we record the last successful check timestamp. Lives next to
- L3004 `_last_integrity_check_ts` `[private]` — Returns the unix timestamp of the most recent successful check, or
- L3015 `_record_integrity_check_ts` `[private]` — Atomic write of the timestamp marker. Best-effort — a failed write
- L3027 `run_integrity_check` — Run PRAGMA integrity_check on a background thread, debounced to
- L3127 `_row_count_estimate` `[private]` — Cheap estimate of total history+queue rows for the log message —
- L3140 `_ensure_host_throughput_table` `[private]` — Idempotently create the per-host throughput table. One row per host,
- L3151 `host_throughput_record` — Upsert the last multi-conn outcome for a host. Best-effort; never raises.
- L3171 `host_throughput_get` — Return {chunk_count, avg_speed_bps, chunks_failed, updated_at} for a host,
```
...
- L0092 `_wait_attached` `[private]` — Wait for ATTACHMENT. The only thing attached may be the decoy, and
- L0098 `_wait_visible` `[private]` — Wait for VISIBILITY, which is what a field must be before we fill
- L0104 `_signal_tabindex` `[private]` — For a form input ``tabindex="-1"`` is a STANDALONE trap signal.
- L0111 `_signal_aria_hidden` `[private]` — A field announced as hidden to assistive tech is not for a human.
- L0118 `_signal_css_hidden` `[private]` — The SHIPPED style vocabulary, consulted rather than restated.
- L0130 `_signal_hidden_attr` `[private]` — The ``hidden`` attribute: no shipped equivalent, browser-only fact.
- L0137 `_signal_type_hidden` `[private]` — ``type=hidden``: likewise browser-only.
- L0144 `_signal_offscreen_box` `[private]` — A negative bounding box -- the ``left:-9999px`` decoy whose inline
- L0169 `_is_honeypot_field` `[private]` — Return ``(is_decoy, reason)`` for a Playwright input locator.
- L0235 `_try_fill` `[private]` — Walk the candidate list; fill the first visible, non-honeypot
- L0313 `_try_click` `[private]` — Same pattern as _try_fill but for clicks. Force=True is used as a
- L0364 `_human_move_to` `[private]` — Phase 15.6: move the mouse to the locator's center along a curved
- L0423 `_css_escape_for_id` `[private]`
- L0432 `_ms_since` `[private]`
```
...
## `bulk_downloader/login_impl/replay.py` (23 entries)
...
- L0159 `keep_pre_submit_screenshot` — Row 722 (operator): the filled form is REVIEWED before a second
- L0189 `member_state_check` — Positive member-state check on the page the run ACTUALLY read.
- L0274 `_read_login_surface` `[private]` — Browser boundary: what the post-submit page SHOWS. Returns
- L0290 `_judge_login_surface` `[private]` — Pure predicate over a surface read. Returns (anonymous, why).
- L0313 `anonymous_surface_check` — Row 722: does the page the run actually read still show the
- L0332 `_path_prefix_match` `[private]` — True if `candidate` equals `prefix` or extends it at a path-segment
- L0344 `_success_url_matches` `[private]` — Decide whether final_url indicates we landed on the configured
- L0414 `success_url_reached` — Row 722: the ONE success-URL predicate for submit.py. Structural
- L0431 `_looks_authenticated` `[private]` — Decide whether a captured cookie jar plausibly belongs to a
- L0486 `replay_saved_login_flow` — Drive a saved cross-origin N-step login flow for this site, if one was
- L0527 `verify_login_replay` — After a successful manual takeover wizard completes, replay
- L0757 `_build_verify_result` `[private]` — Compose a user-facing summary string from the structured
- L0802 `_compute_cookie_expiry_days` `[private]` — Read cookies/<sid>.json and return the minimum days-until-
- L0862 `_attempt_headless_fill_submit` `[private]` — Minimal headless fill+submit using the learned selectors,
```


## `bulk_downloader/login_impl/submit.py` (16 entries)

```
- L0031 `_brand_host` `[private]` — Lower-cased hostname of an http(s) URL, or "" when unmeasurable.
- L0043 `_same_brand_origin` `[private]` — Row 722 (G17): do the login page and the page the submit landed on
- L0058 `_no_nav_verdict` `[private]` — Row 708: decide a login that fired NO navigation.
- L0087 `_staged_password_retry` `[private]` — Two-step (staged) login recovery. Returns (ok, info).
- L0129 `_click_turnstile_checkbox` `[private]` — Row 722 (vip4k.com): a Cloudflare Turnstile widget in CHECKBOX mode
- L0236 `_click_human_button` `[private]` — A visible button/role=button whose whole text is an 'I am human' /
- L0271 `_is_cloudflare_challenge_page` `[private]` — True when the page is a Cloudflare managed-challenge interstitial
- L0316 `clear_cloudflare_challenge` — Row 722 (adulttime): every login URL answers 307->403 with a
- L0414 `_wait_captcha_tokens` `[private]` — Detect and wait for any of the three major invisible captchas to
- L0446 `_try_turnstile_one_click` `[private]` — Perform one operator-enabled local Turnstile checkbox click.
- L0534 `_build_submit_fallbacks` `[private]` — Build the ordered list of submit-button selectors. Order matters —
- L0625 `_submit_login` `[private]` — Try nine independent ways to submit the login form. Each method
- L0900 `_try_check_remember_me` `[private]` — Check the "Remember me" / "Keep me signed in" / "Stay logged in"
- L1056 `_checked_upsell_boxes` `[private]` — Operator (2026-09-15 13:1xZ): "no box is checked by default -- double
- L1076 `_uncheck_upsell_boxes` `[private]` — Uncheck every VISIBLE, checked upsell/cross-sale checkbox before the
- L1123 `do_login` — Robust login. Tries 25 username selectors, 15 password selectors,
```
...
_Total entries: 685 across 22 files._
