...
- L0982 `SiteRunner` `[class]`
  ...
  - L3433 `SiteRunner._wait_for_lazy_video` `[private]` — v3.43.75: wait for a <video> or <source> to appear in the
  - L3462 `SiteRunner._playlist_expand_one` `[private]` — v3.43.75: expand one listing URL into scene URLs.
  - L3517 `SiteRunner._search_site` `[private]` — v3.43.77: search this site for `query`. Returns SearchResult.
  - L3575 `SiteRunner._worker_generation_is_current` `[private]`
  - L3579 `SiteRunner._watch_done` `[private]` — Background overseer thread spawned by start(). Polls the queue
  - L3640 `SiteRunner._finalize_watch_done` `[private]` — Commit retry/final state only if this overseer still owns the run.
  - L3700 `SiteRunner._claim_completion_notification` `[private]` — Atomically claim a still-current completion token for delivery.
  - L3732 `SiteRunner._notify_watch_done_if_current` `[private]` — Deliver a completion token only after an atomic current-state claim.
  - L3763 `SiteRunner._requeue_generation_item` `[private]` — Restore eligible work using the documented lifecycle lock order.
  - L3783 `SiteRunner._generation_item_is_processable` `[private]` — Validate a dequeued item against the current run and job state.
  - L3792 `SiteRunner._claim_worker_item` `[private]` — Atomically claim eligible current-run work immediately pre-process.
  - L3817 `SiteRunner._publish_worker_exception` `[private]` — Publish an exception that escaped one worker attempt.
  - L3842 `SiteRunner._process_worker_url` `[private]` — Claim, map, and process one URL with an unambiguous result.
  - L3863 `SiteRunner._resource_admission_hold` `[private]` — Return a visible hold when a configured resource gate is not safe.
  - L3945 `SiteRunner._worker_loop` `[private]` — One persistent worker thread. Owns its own playwright + browser
  - L4304 `SiteRunner._dismiss_page_gates` `[private]` — Clear configured/generic gates and publish every observed action.
  - L4319 `SiteRunner._page_gates_are_safe` `[private]` — Run/report page gates and hold the job on any UNKNOWN verdict.
  - L4349 `SiteRunner._record_no_identity_proof` `[private]` — Row 701: stamp the RUN RECORD when the winner was admitted without
  - L4375 `SiteRunner._handle_nothing_in_scope` `[private]` — Row 701's distinct outcome: a download control WAS found on this
  - L4409 `SiteRunner._process_one` `[private]` — Process a single URL.
```
...
- L0901 `TransportMixin` `[class]`
  ...
  - L1306 `TransportMixin._try_multi_conn_download` `[private]` — v3.43.74: probe the URL and, if viable, run a parallel
  - L1491 `TransportMixin._looks_like_media` `[private]` — BP-VH1: True if the response is plausibly downloadable MEDIA, by
  - L1524 `TransportMixin._is_streaming_manifest` `[private]` — Is this response a STREAM INDEX rather than a saveable file?
  - L1548 `TransportMixin._winner_url_value` `[private]` — The winner's URL-bearing attribute VALUE, or "".
  - L1582 `TransportMixin._direct_media_route` `[private]` — (media_url, destination_name) if `href` IS the file, else (None, None).
  - L1655 `TransportMixin._stream_route` `[private]` — (manifest_url, destination_name) if `href` is a stream, else (None, None).
  - L1710 `TransportMixin._probe_outcome` `[private]` — BP-VH1: map a probe result to one of done | streaming | non_media | fail.
  - L1735 `TransportMixin._integrity_size_ok` `[private]` — BP-INT (v3.66.284): True if the received byte count satisfies the
  - L1744 `TransportMixin._header_dedup_bytes` `[private]` — Prefix length the header-stage dedup samples; 0 = gate off
  - L1766 `TransportMixin._header_dedup_gate` `[private]` — Sample the staged prefix; on a perceptual match drop the staging
  - L1817 `TransportMixin._promote_or_abort` `[private]` — BP-INT (v3.66.284): atomically promote the ``.part`` to its final
  - L1846 `TransportMixin._do_probe_fetch` `[private]` — GCW probe mode (v3.66.274): the trigger has fired and ``dl.url`` is
  - L1988 `TransportMixin._download_from_revealed_modal` `[private]` — Re-scrape after a score-0 click and take the quality label it revealed.
  - L2038 `TransportMixin._do_download` `[private]` — Click the download button and save the file. Tries the HTTP path
  - L2960 `TransportMixin._http_download` `[private]` — Run one HTTP transfer inside the RAM staging ownership scope.
  - L3025 `TransportMixin._run_http_attempts_with_resume` `[private]` — Row 903: run the mirror-attempt loop, retrying a TRANSIENT
  - L3073 `TransportMixin._http_download_claimed` `[private]` — Stream the file URL to disk via httpx, with progress updates,
  - L3645 `TransportMixin._probe_size` `[private]` — HEAD request to learn Content-Length + Accept-Ranges. Returns
  - L3695 `TransportMixin._http_download_parallel` `[private]` — Download `total` bytes via N parallel HTTP Range requests.
  - L4100 `TransportMixin._current_cap_mbps` `[private]` — Return the current effective speed cap in MB/s.
  - L4133 `TransportMixin._recommended_chunk_bytes` `[private]` — Return a chunk size in bytes, tuned to recent observed throughput.
  - L4157 `TransportMixin._observe_throughput` `[private]` — Update the EWMA throughput tracker after a download. Called
  - L4182 `TransportMixin._dual_stream_fetch_concurrent` `[private]` — Run ``video_fetch()`` and ``audio_fetch()`` on two threads at the
  - L4208 `TransportMixin._mux_streams_lossless` `[private]` — Multiplex separately-retrieved video/audio elementary streams
  - L4228 `TransportMixin._probe_stream_duration_ms` `[private]` — ffprobe the duration of one stream (``"v:0"`` / ``"a:0"``) in
  - L4252 `TransportMixin._verify_av_sync` `[private]` — Verify a muxed container's video and audio tracks are in sync:
  - L4269 `TransportMixin.dual_stream_mux` — Top-level pipeline: concurrently retrieve two separately-fetched
  - L4286 `TransportMixin._run_transport_consumers` `[private]` — Transport-worker entry point for the discovery queue (row 928):
- L4301 `_ManifestConsumers` `[private]`
  - L4302 `_ManifestConsumers.__init__` `[dunder]`
  - L4305 `_ManifestConsumers.join`
- L4312 `consume_manifest_queue` — Consumer side of row 928: ``workers`` threads pull manifests from
- L4360 `HTTP3Transport` `[class]` — HTTP/3 (QUIC) transport engine with instant TCP fallback (Row 853).
  - L4370 `HTTP3Transport.__init__` `[dunder]`
  - L4378 `HTTP3Transport.allocate_stream_id`
  - L4383 `HTTP3Transport.request` — Open a request, negotiating HTTP/3 when enabled, with instant TCP fallback.
- L4406 `_request_download_stream` `[private]` — Open a download stream, negotiating HTTP/3 when enabled with instant TCP fallback.
- L4419 `_extract_scoped_cookies` `[private]` — Extract cookies scoped to file_url; fallback safely for mocks or closed contexts.
```
