...
- L0366 `_bd_boot_before_request` `[private]` — Boot the DB on the first request rather than at import (item 11).
- L0433 `_enter_non_sites_site_lifecycle_transaction` `[private]` — Serialize mutating non-sites routes with deletion of a live site.
- L0466 `_leave_non_sites_site_lifecycle_transaction` `[private]`
- L0481 `_json_error` `[private]`
- L0486 `_on_attribute_error` `[private]`
- L0505 `_on_bad_request` `[private]`
- L0514 `_on_not_found` `[private]`
- L0520 `_on_method_not_allowed` `[private]`
- L0530 `_on_request_too_large` `[private]`
- L0539 `_on_internal_error` `[private]`
- L0553 `_on_request_budget_exceeded` `[private]`
- L0585 `_expected_token` `[private]`
- L0605 `_bd_token_secondary` `[private]` — v3.66.317: optional SECOND accepted server-side token (BD_TOKEN promoted
- L0618 `_accepted_tokens` `[private]` — All server-side tokens that authenticate an /api/ request: the primary
- L0630 `app_test_mode` — v3.66.317: advisory TEST-MODE indicator (BD_TEST_MODE promoted full).
- L0688 `_token_eq` `[private]` — AF1 (v3.66.41): constant-time comparison for auth secrets. Plain
- L0700 `_check_token` `[private]`
- L0892 `_session_valid` `[private]` — True iff the session exists and hasn't expired (idle TTL).
- L0904 `_session_touch` `[private]` — Slide the idle TTL by updating last_used.
- L0911 `_session_create` `[private]` — Mint a new session. source is for diagnostics — "pair_redeem",
- L0921 `_csrf_token_for` `[private]` — Derive a CSRF token from the session token. Deterministic per
- L0966 `csrf_fires_for` — Route-level answer to: would `_check_csrf` 403 a cookie-session browser
- L0986 `_check_csrf` `[private]` — For state-changing requests (POST/PUT/PATCH/DELETE) under /api/,
- L1084 `_add_security_headers` `[private]` — Audit 2026-05 / Phase 2C: defense-in-depth response headers.
- L1138 `_dev_metrics_start` `[private]`
- L1146 `_dev_metrics_record` `[private]`
- L1229 `_bootstrap_session` `[private]` — If the request is for the HTML shell (GET /) and there's no
- L1291 `serve_mobile_view` `GET /m/; GET /m`
- L1308 `serve_mobile_ops_view` `GET /m/ops/; GET /m/ops`
- L1339 `_m2_503_node_missing` `[private]` — Uniform 503 response when frontend/dist/ is missing.
- L1394 `_m2_opt_state` `[private]` — Return one of 'v2', 'v1', or 'default' based on query + cookie.
- L1410 `_m2_apply_opt_cookie` `[private]` — If the request had `?ui=v2` or `?ui=v1`, persist that as the
- L1429 `serve_m2_spa` `GET /m2/; GET /m2; GET /m2/<path:subpath>` — Phase 1 root flip (v3.66.203): the D3 SPA moved from /m2 to `/`
- L1476 `_resolve_sites_file` `[private]` — Where the site config lives.
- L1529 `_sites_file_for_boot` `[private]` — Resolve a candidate without corrupting a bound runtime on rejection.
- L1536 `_publish_sites_file_for_runtime` `[private]` — Publish an accepted auto-owned candidate; preserve explicit patches.
- L1561 `_sites_config_reachability` `[private]` — Return the last load's operator-facing path measurement.
- L1567 `_save_sites_config` `[private]` — Write current sites to disk. Called on every add/update/delete.
- L1654 `_build_meta` `[private]` — Return a copy of cfg with secrets stripped — top-level password
- L1693 `_load_sites_config` `[private]` — Read sites from disk on startup and instantiate SiteRunners.
- L1853 `_init_vpn_runtime` `[private]` — Initialize VPN mappings after configured sites have been restored.
- L1894 `_start_session_keepers` `[private]`
- L2011 `_heartbeat_to_disk_loop` `[private]`
- L2051 `_start_watch_folder_threads` `[private]` — Spawn one daemon thread per configured site. The thread's
- L2136 `_resolved_site_runtime_path` `[private]` — Return the absolute identity of the configured-sites runtime.
- L2145 `_require_compatible_site_runtime_path` `[private]` — Refuse to bind the process's live runners to a second config file.
- L2158 `_retire_configured_runtime_runners` `[private]` — Retire every partially loaded runner without losing any handle.
- L2184 `_retire_configured_runtime_watchers` `[private]` — Signal and prove every configured watch generation quiescent.
- L2248 `_retire_configured_runtime_dependencies` `[private]` — Quiesce watcher/keeper producers without resetting VPN leaves.
- L2279 `_retire_configured_runtime_all` `[private]` — Retire all producers, then and only then reset their VPN leaf.
- L2293 `_clear_configured_runtime_state` `[private]` — Clear a partial load only after every owned generation is dead.
- L2310 `_activate_configured_runtime_once` `[private]` — Restore configured sites and their dependent services exactly once.
- L2392 `_start_window_scheduler` `[private]`
- L2461 `_start_storage_tier_scheduler` `[private]`
- L2480 `_load_app_config` `[private]`
- L2573 `_save_app_config` `[private]` — Persist global app config. Atomic via .tmp + replace (v3.43.19):
- L2635 `_start_background_services` `[private]` — Start bg_scheduler + the webhook drain worker. Called from boot_once().
- L2721 `_routing_config_projection` `[private]` — Return the immutable-by-value inputs used for URL/site scoring.
- L2732 `_route_urls_internal` `[private]` — Internal helper: same routing logic as /api/route_urls but without
- L2795 `_saved_search_enqueue_handler` `[private]`
- L2810 `_score_url_against_sites` `[private]` — v3.43.40: extracted scoring helper used by both routing
- L2902 `_watcher_loop` `[private]` — Thread body. Polls watch_folder for new .txt files; imports URLs;
- L2973 `_start_watcher` `[private]` — Idempotent start. Called at explicit boot + whenever app_config
- L3015 `_stop_watcher` `[private]` — Signal and boundedly prove the process-global watcher generation.
- L3087 `_explicit_methods_for_path` `[private]` — Methods served by EXPLICIT rules (anything but the serve_spa_root
- L3105 `_405_with_allow` `[private]`
- L3113 `_method_not_allowed_parity` `[private]` — Restore pre-flip 404/405 semantics for non-GET requests whose
- L3146 `serve_spa_root` `GET /; GET /<path:subpath>` — Serve the D3 React SPA from frontend/dist/ at the site root.
- L3229 `pwa_manifest` `GET /manifest.json`
- L3245 `pwa_icon_svg` `GET /icon.svg` — Same icon as the favicon — bundled inline so we don't need a static dir.
- L3358 `pwa_sw` `GET /sw.js`
- L3378 `_origins_env_locked` `[private]` — A field is env-locked when a BD_<UPPER(field)> env var pins it. Static
- L3390 `_oi_flagged` `[private]` — Best-effort count of sites whose status looks problematic. Fail-soft:
- L3416 `_oi_dir_writable` `[private]` — (exists, writable) for a candidate dir — read-only, never creates.
- L3425 `_oi_default_download_dir` `[private]` — The directory a site downloads into when it names none of its own.
- L3450 `_chk` `[private]`
- L3571 `_request_is_same_origin` `[private]` — True when the request's Referer host:port matches its Host -- i.e. it
- L3594 `_dev_request_authorized` `[private]` — F-APP04-01: the /api/dev/* surface is privileged (lint / probe /
- L3625 `_dev_mode_guard` `[private]` — Return a 404 response if dev mode is off, or a 403 if the request is not
- L4106 `metrics_endpoint` `GET /metrics` — Prometheus text-format exposition. Scrape-friendly; no auth.
- L4141 `_bd_cookie_dir` `[private]`
- L4148 `_site_primary_url` `[private]` — Resolve a content URL, retaining an explicit login-first mode.
- L4246 `stream_serve` `GET /stream/<token>` — Serve a video file with HTTP range support, gated by token.
- L4332 `_capture_enqueue` `[private]` — Inject seam for capture_schedules.run_*: append URL(s) to a site's
- L4449 `_m2_avatar_color` `[private]` — Deterministic name → color (one of 12 hues). Same input always
- L4462 `_m2_site_drain_eta` `[private]` — F1.6: estimate seconds to drain one site's queue from that site's
- L4480 `_m2_auth_state` `[private]` — Bucket the runner's auth state into ok/expired/unknown.
- L4528 `_m2_attention_for_site` `[private]` — Return an attention-banner entry for a site, or None if it has
- L4578 `_m2_age_human` `[private]` — Compact human age — '2h ago', '15m ago', '3d ago'. Empty if
- L4597 `_m2_honeypot_suggestion` `[private]` — Advisory per-site honeypot drop-threshold suggestion for the
- L4624 `_m2_activity_query_fragments` `[private]` — Build (where_clauses, params) for activity_v2 + export endpoints.
- L4715 `_diff_parse_target` `[private]` — Parse a colon-separated 'site_id:url' from the query string.
- L4731 `_diff_collect_one` `[private]` — Resolve one diff side. Returns a dict with keys site_id, url,
- L4773 `_diff_lines_for` `[private]` — Render an events list as one string per event, formatted
- L4832 `_status_snapshot` `[private]` — Build the same dict shape that /api/status would return. Extracted
- L4864 `_dashboard_snapshot` `[private]` — Build the same dict that /api/dashboard returns.
- L4994 `_validate_path` `[private]` — Returns (ok: bool, normalized_path_or_error_message: str).
- L5039 `_reveal_safe_roots` `[private]` — F-APP06-01: the effective allowlist for the reveal action -- the
- L5074 `_validate_reveal_path` `[private]` — F-APP06-01: reveal-scoped path check. Runs the standard _validate_path
- L5099 `_validate_config_paths` `[private]` — Run _validate_path on every path-bearing field. Returns
- L5133 `_sanitize_display_name` `[private]` — Normalize a user-facing display string. Returns the cleaned value.
- L5153 `_create_site` `[private]` — Create one site from a config dict. Returns (sid, error).
- L5247 `_apply_template_by_id` `[private]` — Merge a template's learned block + config_defaults into a site,
- L5281 `_apply_login_template_by_id` `[private]` — Merge a LOGIN template's selectors into a site's learned.login.
- L5309 `_apply_detected_selectors` `[private]` — v3.66.0: merge selectors discovered by auto_detect.detect_site_config
- L5359 `_auto_pick_templates` `[private]` — v3.65.2: Automatically apply matching login + download templates
- L5556 `_vault_guard_for_password` `[private]` — v3.66.326: gate storing a site login password in the secrets vault.
- L5592 `_store_site_password_in_vault` `[private]` — v3.66.326: store ``password`` for ``sid`` in the secrets vault and
- L5753 `_lan_ip_guess` `[private]` — Best-effort detection of this host's LAN IP. Uses the "connect
- L5817 `_teach_cors_response` `[private]` — Add CORS headers for the takeover browser. The teach overlay
- L5957 `_require_vault_token` `[private]` — Helper that validates the Authorization: Bearer <vault_token>
- L5982 `_reject_if_vault_token` `[private]` — B12 (v3.66.38): management routes (pair_issue / list_paired /
- L6186 `_rate_sweep_locked` `[private]` — Drop bucket entries with no timestamps newer than the window. Caller
- L6199 `_is_url_public` `[private]` — AUDIT FIX (v3.43.16): SSRF defence. Resolve the hostname and ensure
- L6226 `_rate_check` `[private]` — Return True if the request should be allowed, False if rate
- L6246 `_do_action` `[private]` — Common body for start/pause/resume/stop/clear/retry. Rate-limits
- L6286 `_do_action_all` `[private]` — Apply `action` to every runner. Returns aggregate result.
- L6354 `_validate_bulk_urls` `[private]` — Common URL list validation. Returns (ok, urls_or_error_dict).
- L6496 `serve_ss` `GET /screenshots/<path:filename>`
- L6805 `_global_notify_settings_path` `[private]` — Where the GLOBAL apprise settings live (not per-site).
- L6812 `_load_global_notify_settings` `[private]` — Load global apprise settings from disk. Fail-open.
- L6823 `_save_global_notify_settings` `[private]` — Persist global apprise settings. Fail-open. Atomic write so a
- L6838 `_apply_global_notify_config` `[private]` — Push the saved settings into the dispatcher singleton.
- L6902 `_tg_get_status` `[private]` — Callback for /status: build the site overview dict.
- L6931 `_tg_get_queue` `[private]` — Callback for /queue: return the queue for a site.
- L6939 `_tg_add_url` `[private]` — Callback for /mirror: auto-route and add.
- L6956 `_tg_cancel_url` `[private]` — Callback for /cancel: find the matching pending job and mark it
- L6968 `_tg_retry_site` `[private]` — Callback for /retry: reset failed→pending in one site or all.
- L6991 `_tg_pause_site` `[private]` — Callback for /pause: pause one site or all.
- L7007 `_tg_resume_site` `[private]` — Callback for /resume: resume one site or all.
- L7023 `_persist_cfg` `[private]` — Helper to save s_cfg back to disk.
- L7040 `_apply_tg_bot_config` `[private]` — Push saved settings into the bot singleton.
- L7093 `_dedup_get_registry` `[private]` — Get/create the singleton registry. Picks DB path from any site's
- L7214 `_serialize_search_result` `[private]` — Convert a SearchResult dataclass to a JSON-friendly dict.
```


## `bulk_downloader/runner.py` (89 entries)
...
- L1005 `SiteRunner` `[class]`
  ...
  - L2064 `SiteRunner._publish_watchdog_snapshot` `[private]` — Publish only a still-current heartbeat snapshot for this run.
  - L2074 `SiteRunner._watchdog_loop` `[private]` — v3.43.24: monitor worker heartbeats. Threads should stamp
  - L2135 `SiteRunner._effective_concurrency` `[private]` — Phase 64 (v3.41.0): bandwidth-aware concurrency. If
  - L2213 `SiteRunner.drain` — Drain in-flight workers to safe transactional boundary and pause (Row 1075).
  - L2226 `SiteRunner.pause` — Pause the worker pool. Workers finish the URL they're currently
  - L2254 `SiteRunner.resume` — Resume from paused / paused_no_button / low_disk states.
  - L2304 `SiteRunner.stop`
  - L2408 `SiteRunner.retire_workers` — Permanently stop and prove every runner-owned writer quiescent.
  - L2731 `SiteRunner._current_throughput_bps` `[private]` — Sum recent byte rates for jobs that are still running.
  - L2755 `SiteRunner.get_status` — Return runner state. With `light=True`, omit `jobs` and
  - L2860 `SiteRunner._learned_summary` `[private]` — Compact summary for the UI: which kinds are learned, how many
  - L2877 `SiteRunner.state`
  - L2879 `SiteRunner._compute_site_usage` `[private]` — Phase 65 (v3.38.x): sum the byte size of all files under
  - L2915 `SiteRunner._worker_write_generation` `[private]` — Return a worker-thread generation, or None for control-plane writes.
  - L2922 `SiteRunner._worker_write_generation_is_current` `[private]` — Reject mutations from worker threads whose run was invalidated.
  - L2929 `SiteRunner._capture_website_title` `[private]` — Harvest a settled detail page once and retain its provenance.
  - L2995 `SiteRunner._history_title_fields` `[private]` — Return db_log kwargs without inventing a title from a filename.
  - L3026 `SiteRunner._update_job` `[private]` — Serialize worker-originated publication against stop/start.
  - L3047 `SiteRunner._update_job_current` `[private]` — Central state-mutation: change a job's status/message, log
  - L3635 `SiteRunner._wait_for_lazy_video` `[private]` — v3.43.75: wait for a <video> or <source> to appear in the
  - L3664 `SiteRunner._playlist_expand_one` `[private]` — v3.43.75: expand one listing URL into scene URLs.
  - L3719 `SiteRunner._search_site` `[private]` — v3.43.77: search this site for `query`. Returns SearchResult.
  - L3777 `SiteRunner._worker_generation_is_current` `[private]`
  - L3781 `SiteRunner._watch_done` `[private]` — Background overseer thread spawned by start(). Polls the queue
  - L3842 `SiteRunner._finalize_watch_done` `[private]` — Commit retry/final state only if this overseer still owns the run.
  - L3902 `SiteRunner._claim_completion_notification` `[private]` — Atomically claim a still-current completion token for delivery.
  - L3934 `SiteRunner._notify_watch_done_if_current` `[private]` — Deliver a completion token only after an atomic current-state claim.
  - L3965 `SiteRunner._requeue_generation_item` `[private]` — Restore eligible work using the documented lifecycle lock order.
  - L3985 `SiteRunner._generation_item_is_processable` `[private]` — Validate a dequeued item against the current run and job state.
  - L3994 `SiteRunner._claim_worker_item` `[private]` — Atomically claim eligible current-run work immediately pre-process.
  - L4019 `SiteRunner._publish_worker_exception` `[private]` — Publish an exception that escaped one worker attempt.
  - L4044 `SiteRunner._process_worker_url` `[private]` — Claim, map, and process one URL with an unambiguous result.
  - L4073 `SiteRunner._resource_admission_hold` `[private]` — Return a visible hold when a configured resource gate is not safe.
  - L4155 `SiteRunner._worker_loop` `[private]` — One persistent worker thread. Owns its own playwright + browser
  - L4521 `SiteRunner._dismiss_page_gates` `[private]` — Clear configured/generic gates and publish every observed action.
  - L4536 `SiteRunner._page_gates_are_safe` `[private]` — Run/report page gates and hold the job on any UNKNOWN verdict.
  - L4566 `SiteRunner._record_no_identity_proof` `[private]` — Row 701: stamp the RUN RECORD when the winner was admitted without
  - L4592 `SiteRunner._handle_nothing_in_scope` `[private]` — Row 701's distinct outcome: a download control WAS found on this
  - L4626 `SiteRunner._process_one` `[private]` — Process a single URL.
```
...
- L0901 `TransportMixin` `[class]`
  ...
  - L1519 `TransportMixin._looks_like_media` `[private]` — BP-VH1: True if the response is plausibly downloadable MEDIA, by
  - L1552 `TransportMixin._is_streaming_manifest` `[private]` — Is this response a STREAM INDEX rather than a saveable file?
  - L1576 `TransportMixin._winner_url_value` `[private]` — The winner's URL-bearing attribute VALUE, or "".
  - L1610 `TransportMixin._direct_media_route` `[private]` — (media_url, destination_name) if `href` IS the file, else (None, None).
  - L1683 `TransportMixin._stream_route` `[private]` — (manifest_url, destination_name) if `href` is a stream, else (None, None).
  - L1738 `TransportMixin._probe_outcome` `[private]` — BP-VH1: map a probe result to one of done | streaming | non_media | fail.
  - L1763 `TransportMixin._integrity_size_ok` `[private]` — BP-INT (v3.66.284): True if the received byte count satisfies the
  - L1772 `TransportMixin._header_dedup_bytes` `[private]` — Prefix length the header-stage dedup samples; 0 = gate off
  - L1794 `TransportMixin._header_dedup_gate` `[private]` — Sample the staged prefix; on a perceptual match drop the staging
  - L1845 `TransportMixin._promote_or_abort` `[private]` — BP-INT (v3.66.284): atomically promote the ``.part`` to its final
  - L1874 `TransportMixin._do_probe_fetch` `[private]` — GCW probe mode (v3.66.274): the trigger has fired and ``dl.url`` is
  - L2016 `TransportMixin._download_from_revealed_modal` `[private]` — Re-scrape after a score-0 click and take the quality label it revealed.
  - L2066 `TransportMixin._do_download` `[private]` — Click the download button and save the file. Tries the HTTP path
  - L2993 `TransportMixin._http_download` `[private]` — Run one HTTP transfer inside the RAM staging ownership scope.
  - L3058 `TransportMixin._run_http_attempts_with_resume` `[private]` — Row 903: run the mirror-attempt loop, retrying a TRANSIENT
  - L3106 `TransportMixin._http_download_claimed` `[private]` — Stream the file URL to disk via httpx, with progress updates,
  - L3688 `TransportMixin._probe_size` `[private]` — HEAD request to learn Content-Length + Accept-Ranges. Returns
  - L3738 `TransportMixin._http_download_parallel` `[private]` — Download `total` bytes via N parallel HTTP Range requests.
  - L4143 `TransportMixin._current_cap_mbps` `[private]` — Return the current effective speed cap in MB/s.
  - L4176 `TransportMixin._recommended_chunk_bytes` `[private]` — Return a chunk size in bytes, tuned to recent observed throughput.
  - L4200 `TransportMixin._observe_throughput` `[private]` — Update the EWMA throughput tracker after a download. Called
  - L4225 `TransportMixin._dual_stream_fetch_concurrent` `[private]` — Run ``video_fetch()`` and ``audio_fetch()`` on two threads at the
  - L4251 `TransportMixin._mux_streams_lossless` `[private]` — Multiplex separately-retrieved video/audio elementary streams
  - L4271 `TransportMixin._probe_stream_duration_ms` `[private]` — ffprobe the duration of one stream (``"v:0"`` / ``"a:0"``) in
  - L4295 `TransportMixin._verify_av_sync` `[private]` — Verify a muxed container's video and audio tracks are in sync:
  - L4312 `TransportMixin.dual_stream_mux` — Top-level pipeline: concurrently retrieve two separately-fetched
  - L4329 `TransportMixin._run_transport_consumers` `[private]` — Transport-worker entry point for the discovery queue (row 928):
- L4344 `_ManifestConsumers` `[private]`
  - L4345 `_ManifestConsumers.__init__` `[dunder]`
  - L4348 `_ManifestConsumers.join`
- L4355 `consume_manifest_queue` — Consumer side of row 928: ``workers`` threads pull manifests from
- L4403 `HTTP3Transport` `[class]` — HTTP/3 (QUIC) transport engine with instant TCP fallback (Row 853).
  - L4413 `HTTP3Transport.__init__` `[dunder]`
  - L4421 `HTTP3Transport.allocate_stream_id`
  - L4426 `HTTP3Transport.request` — Open a request, negotiating HTTP/3 when enabled, with instant TCP fallback.
- L4449 `_request_download_stream` `[private]` — Open a download stream, negotiating HTTP/3 when enabled with instant TCP fallback.
- L4462 `_extract_scoped_cookies` `[private]` — Extract cookies scoped to file_url; fallback safely for mocks or closed contexts.
```


## `bulk_downloader/db.py` (133 entries)
...
- L3477 `_ensure_host_throughput_table` `[private]` — Idempotently create the per-host throughput table. One row per host,
- L3488 `host_throughput_record` — Upsert the last multi-conn outcome for a host. Best-effort; never raises.
- L3508 `host_throughput_get` — Return {chunk_count, avg_speed_bps, chunks_failed, updated_at} for a host,
- L3527 `db_bulk_ingest_staging`
- L3538 `db_staging_ingest_stats`
```
...
_Total entries: 811 across 22 files._
