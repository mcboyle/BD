...
- L0370 `_bd_boot_before_request` `[private]` — Boot the DB on the first request rather than at import (item 11).
- L0437 `_enter_non_sites_site_lifecycle_transaction` `[private]` — Serialize mutating non-sites routes with deletion of a live site.
- L0470 `_leave_non_sites_site_lifecycle_transaction` `[private]`
- L0485 `_json_error` `[private]`
- L0490 `_on_attribute_error` `[private]`
- L0509 `_on_bad_request` `[private]`
- L0518 `_on_not_found` `[private]`
- L0524 `_on_method_not_allowed` `[private]`
- L0534 `_on_request_too_large` `[private]`
- L0543 `_on_internal_error` `[private]`
- L0560 `_on_request_budget_exceeded` `[private]`
- L0592 `_expected_token` `[private]`
- L0612 `_bd_token_secondary` `[private]` — v3.66.317: optional SECOND accepted server-side token (BD_TOKEN promoted
- L0625 `_accepted_tokens` `[private]` — All server-side tokens that authenticate an /api/ request: the primary
- L0637 `app_test_mode` — v3.66.317: advisory TEST-MODE indicator (BD_TEST_MODE promoted full).
- L0695 `_token_eq` `[private]` — AF1 (v3.66.41): constant-time comparison for auth secrets. Plain
- L0707 `_check_token` `[private]`
- L0899 `_session_valid` `[private]` — True iff the session exists and hasn't expired (idle TTL).
- L0911 `_session_touch` `[private]` — Slide the idle TTL by updating last_used.
- L0918 `_session_create` `[private]` — Mint a new session. source is for diagnostics — "pair_redeem",
- L0928 `_csrf_token_for` `[private]` — Derive a CSRF token from the session token. Deterministic per
- L0973 `csrf_fires_for` — Route-level answer to: would `_check_csrf` 403 a cookie-session browser
- L0993 `_check_csrf` `[private]` — For state-changing requests (POST/PUT/PATCH/DELETE) under /api/,
- L1091 `_add_security_headers` `[private]` — Audit 2026-05 / Phase 2C: defense-in-depth response headers.
- L1145 `_dev_metrics_start` `[private]`
- L1153 `_dev_metrics_record` `[private]`
- L1236 `_bootstrap_session` `[private]` — If the request is for the HTML shell (GET /) and there's no
- L1298 `serve_mobile_view` `GET /m/; GET /m`
- L1315 `serve_mobile_ops_view` `GET /m/ops/; GET /m/ops`
- L1346 `_m2_503_node_missing` `[private]` — Uniform 503 response when frontend/dist/ is missing.
- L1401 `_m2_opt_state` `[private]` — Return one of 'v2', 'v1', or 'default' based on query + cookie.
- L1417 `_m2_apply_opt_cookie` `[private]` — If the request had `?ui=v2` or `?ui=v1`, persist that as the
- L1436 `serve_m2_spa` `GET /m2/; GET /m2; GET /m2/<path:subpath>` — Phase 1 root flip (v3.66.203): the D3 SPA moved from /m2 to `/`
- L1483 `_resolve_sites_file` `[private]` — Where the site config lives.
- L1536 `_sites_file_for_boot` `[private]` — Resolve a candidate without corrupting a bound runtime on rejection.
- L1543 `_publish_sites_file_for_runtime` `[private]` — Publish an accepted auto-owned candidate; preserve explicit patches.
- L1568 `_sites_config_reachability` `[private]` — Return the last load's operator-facing path measurement.
...
- L2399 `_start_window_scheduler` `[private]`
- L2468 `_start_storage_tier_scheduler` `[private]`
- L2487 `_load_app_config` `[private]`
- L2580 `_save_app_config` `[private]` — Persist global app config. Atomic via .tmp + replace (v3.43.19):
- L2642 `_start_background_services` `[private]` — Start bg_scheduler + the webhook drain worker. Called from boot_once().
- L2728 `_routing_config_projection` `[private]` — Return the immutable-by-value inputs used for URL/site scoring.
- L2739 `_route_urls_internal` `[private]` — Internal helper: same routing logic as /api/route_urls but without
- L2802 `_saved_search_enqueue_handler` `[private]`
- L2817 `_score_url_against_sites` `[private]` — v3.43.40: extracted scoring helper used by both routing
- L2909 `_watcher_loop` `[private]` — Thread body. Polls watch_folder for new .txt files; imports URLs;
- L2980 `_start_watcher` `[private]` — Idempotent start. Called at explicit boot + whenever app_config
- L3022 `_stop_watcher` `[private]` — Signal and boundedly prove the process-global watcher generation.
- L3094 `_explicit_methods_for_path` `[private]` — Methods served by EXPLICIT rules (anything but the serve_spa_root
- L3112 `_405_with_allow` `[private]`
- L3120 `_method_not_allowed_parity` `[private]` — Restore pre-flip 404/405 semantics for non-GET requests whose
- L3153 `serve_spa_root` `GET /; GET /<path:subpath>` — Serve the D3 React SPA from frontend/dist/ at the site root.
- L3236 `pwa_manifest` `GET /manifest.json`
- L3252 `pwa_icon_svg` `GET /icon.svg` — Same icon as the favicon — bundled inline so we don't need a static dir.
- L3365 `pwa_sw` `GET /sw.js`
- L3385 `_origins_env_locked` `[private]` — A field is env-locked when a BD_<UPPER(field)> env var pins it. Static
- L3397 `_oi_flagged` `[private]` — Best-effort count of sites whose status looks problematic. Fail-soft:
- L3423 `_oi_dir_writable` `[private]` — (exists, writable) for a candidate dir — read-only, never creates.
- L3432 `_oi_default_download_dir` `[private]` — The directory a site downloads into when it names none of its own.
- L3457 `_chk` `[private]`
- L3578 `_request_is_same_origin` `[private]` — True when the request's Referer host:port matches its Host -- i.e. it
- L3601 `_dev_request_authorized` `[private]` — F-APP04-01: the /api/dev/* surface is privileged (lint / probe /
- L3632 `_dev_mode_guard` `[private]` — Return a 404 response if dev mode is off, or a 403 if the request is not
- L4113 `metrics_endpoint` `GET /metrics` — Prometheus text-format exposition. Scrape-friendly; no auth.
- L4148 `_bd_cookie_dir` `[private]`
- L4155 `_site_primary_url` `[private]` — Resolve a content URL, retaining an explicit login-first mode.
- L4253 `stream_serve` `GET /stream/<token>` — Serve a video file with HTTP range support, gated by token.
...
- L0030 `TelemetryMixin` `[class]`
  ...
  - L0126 `TelemetryMixin.record_transfer_completion` — Row 1056: feed one completed file's wire throughput to the bottleneck detector.
  - L0138 `TelemetryMixin.record_workload_observation` — Record an operational workload observation for automated bottleneck detection.
  - L0144 `TelemetryMixin.check_workload_bottlenecks` — Return active workload bottleneck anomalies detected for this runner/site.
  - L0152 `TelemetryMixin.get_events` — Return events with seq > after_seq, optionally filtered by URL
  - L0167 `TelemetryMixin._install_event_listeners` `[private]` — Phase 13.5/13.6: hook page-level Playwright events to capture
  - L0258 `TelemetryMixin._flush_fingerprint_observation` `[private]` — Read back which fingerprinting APIs the page invoked (set by the
  - L0276 `TelemetryMixin._parse_hm` `[private]` — Parse 'HH:MM' to minutes-since-midnight. On error, return default.
  - L0283 `TelemetryMixin._extract_host` `[private]` — Return just the hostname for log messages.
  - L0290 `TelemetryMixin._pick_fastest_mirror` `[private]` — Phase 69 (v3.41.0): speculative mirror failover. Fire concurrent
  - L0371 `TelemetryMixin._build_mirror_urls` `[private]` — Generate alternate URLs to try when the primary CDN fails.
  - L0412 `TelemetryMixin._classify_error` `[private]` — Phase 6.3: classify a failure message into a retry category.
  - L0444 `TelemetryMixin._config_selector_syntax_error` `[private]` — Row 722 (G25b): classify a worker exception as an invalid CONFIG
  - L0477 `TelemetryMixin._handle_failure` `[private]` — Fence all worker failure side effects within one run transaction.
  - L0501 `TelemetryMixin._handle_failure_current` `[private]` — Central failure handler. Classifies the error message into one of
  - L0559 `TelemetryMixin._screenshot` `[private]` — Save a viewport screenshot of `page` to a deterministic filename
  - L0576 `TelemetryMixin.create_trace_span` — Row 1061: OpenTelemetry trace span builder for runner telemetry.
- L0586 `start_url_trace_span` — Row 1061: start the per-URL "runner.process_url" span, or return None.
- L0610 `end_url_trace_span` — Row 1061: end a span from start_url_trace_span; None has nothing to end.
```
