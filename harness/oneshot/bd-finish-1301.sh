#!/bin/bash
# Merge v1301 when CI greens (the chain does not return after a ship refusal),
# then deploy the fleet once the lane is quiet.
set -uo pipefail
A=/home/mboyle/fleet-run-artifacts/2026-08-25; L="$A/FINISH.log"; R=/home/mboyle/BulkDownloader
say(){ printf '%s [finish-1301] %s\n' "$(date -u +%H:%M:%S)" "$*" | tee -a "$L"; }
say "watching v1301 to merge, then fleet deploy"
merged=0
for _ in $(seq 1 240); do
  ...
  MV=${V##*.}
  if [ -n "$V" ] && [ "${MV:-0}" -ge 1301 ] 2>/dev/null; then merged=1; say "main reached $V"; break; fi
  PR=$(gh pr list --repo mcboyle/BD --state open --json number,headRefName \
        -q '.[]|select(.headRefName|startswith("cut/1301"))|.number' 2>/dev/null | head -1)
  if [ -n "$PR" ]; then
    ...
    if [ -n "$ST" ] && ! printf '%s\n' "$ST" | grep -q pending; then
      if printf '%s\n' "$ST" | grep -q fail; then
        ...
        say "PR #$PR green -- merging"; gh pr merge "$PR" --repo mcboyle/BD --merge >>"$L" 2>&1 && say "merged PR #$PR"
      fi
    fi
  fi
  sleep 30
done
[ "$merged" = 1 ] || { say "GAVE UP: v1301 never merged; NO deploy"; exit 1; }
for _ in $(seq 1 90); do
  pgrep -f 'bd-verify-cut\.sh|bd-row-chain\.sh|bd-integrate-row\.sh' >/dev/null 2>&1 || break
  sleep 20
done
say "lane quiet -- deploying the fleet"
if bash /home/mboyle/bd-fleet-deploy.sh >>"$A/deploy/finish1301-$(date -u +%Y%m%dT%H%M%SZ).log" 2>&1; then
