...
say(){ printf '%s [finish-1307] %s\n' "$(date -u +%H:%M:%S)" "$*" | tee -a "$L"; }
say "watching v1307 to merge, then fleet deploy"
merged=0
for _ in $(seq 1 240); do
  ...
  if [ -n "$V" ] && [ "${MV:-0}" -ge 1307 ] 2>/dev/null; then merged=1; say "main reached $V"; break; fi
  PR=$(gh pr list --repo mcboyle/BD --state open --json number,headRefName \
        -q '.[]|select(.headRefName|startswith("cut/1307"))|.number' 2>/dev/null | head -1)
  if [ -n "$PR" ]; then
...
[ "$merged" = 1 ] || { say "GAVE UP: v1307 never merged"; exit 1; }
...
bash /home/mboyle/bd-fleet-deploy.sh >>"$A/deploy/finish1307-$(date -u +%Y%m%dT%H%M%SZ).log" 2>&1 \
