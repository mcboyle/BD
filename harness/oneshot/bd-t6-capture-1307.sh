...
timeout 60 ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no "$IP" \
  'cd ~/BulkDownloader && git checkout -q main && git pull -q --ff-only; git rev-parse --short HEAD' >> "$L" 2>&1
say "=== v1307 CAPTURE COMPLETE, host returned to main ==="
