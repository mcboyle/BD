...
export function CaptureWorkflow() {
  ...
  async function promote() {
    ...
    try {
      // promote_draft -> _safe_name REQUIRES the draft suffix, and the
      // backend answers `400 invalid draft filename` without it. Derive the
      // name here rather than forwarding stagedDraftFile unchecked: that value
      // comes from the build step and is not guaranteed to carry the suffix,
      // so passing it through silently traded a guarantee for a hope.
      const promoteFile = draftFileForGate.endsWith(".template-draft.json")
        ? draftFileForGate
        : `${draftFileForGate.replace(/(\.template)?(-draft)?\.json$/, "")}.template-draft.json`;
